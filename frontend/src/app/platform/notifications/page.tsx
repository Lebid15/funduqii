...
/**
 * Platform-owner notification centre (notifications closure). Inbox only — the
 * owner's own PLATFORM-scoped notifications (new hotels, subscription
 * lifecycle). No hotel context, no external channels; opening a notification
 * follows its internal platform link.
 */
export default function PlatformNotificationsPage() {
  ...
  }, [page, archived, onlyUnread, category, severity, t]);
  ...
  const columns: Column<HotelNotification>[] = [
    ...
    },
    {
      ...
      render: (r) => (
        <Badge tone={severityTone(r.severity)}>{n.severities[r.severity]}</Badge>
      ),
    },
    {
      key: "created_at",
        ...
        <div className="table__actions">
          ...
          {!r.is_read ? (
            <Button
              ...
              loading={busyId === r.id}
              onClick={() =>
                run(r.id, () => markPlatformNotificationRead(r.id), n.inbox.readMsg)
              }
            >
              {n.inbox.markRead}
            </Button>
          ...
          {!r.is_archived ? (
            <Button
              ...
              loading={busyId === r.id}
              onClick={() =>
                run(r.id, () => archivePlatformNotification(r.id), n.inbox.archivedMsg)
              }
            >
              {n.inbox.archiveBtn}
            </Button>
  ...
  return (
    <PageContainer>
      <PageHeader
        title={t.platformNotifications.title}
        subtitle={t.platformNotifications.subtitle}
      />
      <Card>
        <div className="cluster" style={{ justifyContent: "flex-end" }}>
          <Button
            ...
            icon={CheckCheck}
            onClick={() =>
              run(0, () => markAllPlatformNotificationsRead(), n.inbox.allReadMsg)
            }
          >
            {n.inbox.markAllRead}
          </Button>
        ...
        <FilterBar>
          <FormField label={n.filters.category} htmlFor="pnf-category">
            <Select
              id="pnf-category"
              value={category}
          ...
          <FormField label={n.filters.severity} htmlFor="pnf-severity">
            <Select
              id="pnf-severity"
              value={severity}
              placeholder={t.common.all}
