...
- الدمج: PR #35 · squash · merge commit `1e1921e` · اعتماد المالك النهائي — الفرع محذوف محليًا وبعيدًا · **القسم السابع والأخير — برنامج إغلاق الأقسام السبعة مكتمل**
...
- التاريخ: 2026-07-12 · الأساس: `origin/main@9d679d2` · الفرع: `claude/daily-close-final-closure` · منفذة بأمر المالك بعد اعتماد البحث المرجعي والـ Audit والخطة (بقراراتها المعتمدة) — **Migration واحدة مبررة، صفر صلاحيات جديدة**، والمعمار القائم (Phase 12) مصدر الحقيقة.
- **1) business_date مخزَّن مركزيًا:** أُضيف `HotelSettings.business_date` (DateField)؛ `get_business_date` صار يقرأ القيمة المخزَّنة (‏fallback قرائي لـ`timezone.localdate` عند null — بلا حفظ، فيبقى Prepare قرائيًا)؛ اليوم التشغيلي لم يعد يتبع الساعة، وينتقل فقط بـClose (+1 يوم). لا قبول من العميل، لا واجهة تعديل يدوي، لا قفز/رجوع.
- **2) Migration واحدة + Backfill آمن:** إضافة الحقل + `RunPython` بـ**فحص قبلي** (تاريخ إغلاق مستقبلي/DRAFT مستقبلي/timezone غير صالح/مرشح أقدم من آخر إغلاق) يوقف بصوتٍ عالٍ عند الشذوذ بلا إصلاح صامت؛ Backfill: `أحدث business_date مغلق + 1` وإلا `localdate` بتوقيت الفندق. **لا مساس بأي تاريخ مالي/تشغيلي قديم.**
- **3) القفل المركزي (أقل تعديل ممكن):** كل ختم لتاريخ العمل يمرّ عبر `get_business_date`؛ أُضيف `lock_business_date` (‏`select_for_update` على HotelSettings) و`roll` داخل Close؛ وُحِّد في `finance._business_date` و`services._business_date` فصار كل كتابة مالية/طلبية تقرأ التاريخ **تحت قفل الصف** — نقطة تسلسل واحدة بين الكتابة والإغلاق. ترتيب موحّد: HotelSettings → DailyClose → موارد الخدمة → التسلسلات (لا دورة انتظار لأن الإغلاق لا يقفل أي فوليو).
- **4) Prepare قراءة فقط:** لا DailyClose/DRAFT/Snapshot/نشاط/تغيير تاريخ؛ يعيد `{business_date, can_close, blocking_errors, warnings, informational_alerts, preview_totals}` من الحيّ، متكرّر بلا أثر.
- **5) التصنيف الثلاثي:** المانع الوحيد المطلق = **وردية مفتوحة** (بلا Override)؛ **التسليم المعلّق حُوّل من مانع إلى تحذير**؛ تحذيرات: مغادرة متأخرة/وصول غير مسجّل/طلب مطعم مفتوح؛ معلومات: فوليوهات مفتوحة وأرصدة/حركات غير مسندة/تدبير-صيانة-مفقودات/غرف غير جاهزة. لا تغيير لأي حالة أثناء Prepare أو Close.
- **6) Close ذرّي (‏`transaction.atomic`):** قفل HotelSettings → قراءة التاريخ → التحقق (= الجاري، لا مستقبلي/قديم → `business_date_mismatch`) → قفل/جلب DailyClose (رفض المغلق `day_already_closed`، إعادة استخدام DRAFT الجاري) → إعادة فحص المانع تحت القفل → بناء Snapshot النهائي → تثبيت closed/closed_by/at/snapshot/totals → **نقل business_date +1** → حدث `daily_close.closed` مُثرى (‏old→new + عدّ التحذيرات/المعلومات/غير المسندة). أي فشل ⇒ Rollback كامل (لا نصف-إغلاق، لا نصف-Snapshot، لا نقل، لا حدث زائف).
- **7) Snapshot النهائي الموسّع الثابت (8 مجموعات):** الهوية (‏business_date/previous/next/timezone/currency) · الورديات (مغلقة/ملغاة/الأرصدة/expected-actual-difference/ذات الفرق) · الدفعات (بالطريقة/نقدي-غير نقدي/voided/**عكوسات منفصلة**) · المصروفات (بالطريقة والفئة/**عكوسات منفصلة**) · المطعم-الكافيه (مبيعات بالمنفذ/direct/folio/مفتوحة/ملغاة) · الفوليوهات (مفتوحة/موجب-سالب-صفري بأرصدتها/مغلقة اليوم/**عملات أجنبية منفصلة بلا خلط**) · التشغيل (in_house/وصول غير مسجّل/مغادرة متأخرة/تدبير-صيانة-غرف-مفقودات) · الاستثناءات (blocking/warnings/informational + حركات غير مسندة مفصّلة أصل≠عكس + صافٍ). إجماليات/عدّادات/مراجع فقط. المغلق يُقرأ من `snapshot_json` دون إعادة حساب.
- **8) الطباعة:** `GET shifts/daily-close/<pk>/statement/` تحت `daily_close.view` — `document="daily_close_statement"` من `snapshot_json`/`totals_json` المخزَّنين فقط (لا إعادة حساب حيّ)، قابل لإعادة الطباعة عبر بنية الطباعة القائمة.
- **صفر:** ترحيل رسوم غرفة (الإغلاق لا يُنشئ أي رسم — مثبت) · إعادة احتساب ضريبة · Reopen (‏reopen/run تبقيان Legacy) · Override للوردية · صلاحيات جديدة · تعديل RBAC/Route Guards.
- ‏Migrations: **1** (‏`hotels/0005_hotelsettings_business_date`). صلاحيات جديدة: **صفر**.
- اختبارات: ‏backend من 983 إلى **1002** (اختبارات إغلاق يومي مُعاد بناؤها + business_date/Prepare قرائي/التصنيف/النقل/Idempotency/mismatch/Snapshot/عكوسات/فوليوهات/لا رسوم/لا ضريبة/طباعة/صلاحيات/عزل/timezone/tzبackfill/تراجع بفشل)؛ ‏`makemigrations --check` نظيف.
- سبر حي معزول بفندقين مؤقتين (HTTP + خدمات): **‏19/19** — تنظيف اعتمادي كامل مثبت بتطابق العدّ قبل/بعد.

---

## Task — Shifts Final Closure: جولة الإغلاق النهائية لقسم الورديات
- الحالة: **مكتملة ✅ ومعتمدة ومدموجة** (جولة إحكام صغيرة — ليست مرحلة)
- الدمج: PR #33 · squash · merge commit `646dea0` · اعتماد المالك النهائي — الفرع محذوف محليًا وبعيدًا
- التاريخ: 2026-07-11 · الأساس: `origin/main@38acc29` · الفرع: `claude/shifts-final-closure` · منفذة بأمر المالك بعد اعتماد البحث المرجعي والـ Audit والخطة (بقراراتها الـ 10) — **صفر Migrations، صفر صلاحيات جديدة**، والمعمار القائم (Phase 12) مصدر الحقيقة بلا أي إعادة بناء.
- **1) الوردية المغلقة/الملغاة read-only بالكامل:** `update_shift` صار `editable=()` لأي حالة غير `open` — يُرفض **كل** حقل (بما فيه `internal_notes`) بـ**`operation_not_editable` (409)**؛ سُدَّ الثقب الوحيد الذي كان يسمح بتعديل الملاحظة الداخلية بعد الإغلاق (قرار 1، 6). الوردية المفتوحة تبقى قابلة للتعديل المحدود (float/notes) بلا تغيير.
- **2) استكمال سجل النشاط (6 أحداث جديدة):** ‏`shift.opened` (المسؤول/الرصيد/business_date) · `shift.cancelled` (بالسبب) · `handover.submitted` (من→إلى) · `handover.accepted` (بالملاحظة) · `handover.rejected` (بالسبب) · `handover.cancelled` (بالسبب) — عبر `record_activity` القائم بـ`category="shift"` والفاعل والوقت والقيم؛ الحدثان القائمان `shift.closed`/`daily_close.closed` بلا تغيير؛ سجلات الحالة الخاصة باقية.
- **3) الطباعة (بنية الطباعة القائمة فقط):** ‏`GET shifts/{id}/statement/` (كشف الوردية: الحالة/المسؤول/business_date/الفتح والإغلاق/الرصيد الافتتاحي/**المتوقع والفعلي والفرق وسببه** + ملخص الدرج المشتق + الحركات غير المسندة) و`GET shifts/handovers/{id}/voucher/` (سند التسليم: من الوردية→المستلِم + الملاحظات الست + التواريخ) — كلاهما `{document, hotel, ...}` قابل لإعادة الطباعة.
- **4) الطباعة تحت `shifts.view` — صفر صلاحيات جديدة** (قرار 4).
- **صفر تغيير** في `expected_cash`/`actual_cash`/`business_date`/الحركات غير المسندة/الإغلاق اليومي/أي خدمة مالية مركزية (قرارات 5–8)؛ لا إعادة فتح للورديات؛ لا Migration (قرار 10).
- ‏Migrations: **صفر** (`makemigrations --check` = No changes detected). صلاحيات جديدة: **صفر**.
- اختبارات: ‏backend من 972 إلى **983** (+11: نهائية المغلق 3، الأحداث 4، الطباعة 4؛ مع تكييف اختبار واحد قائم للسلوك read-only الكامل)؛ ‏i18n ‏2337→**2343×3** بتكافؤ تام (+6، بإعادة استخدام مفاتيح الورديات القائمة).
- سبر حي معزول بفندقين مؤقتين (HTTP عبر APIClient): **‏13/13** سيناريو — تنظيف اعتمادي كامل مثبت بتطابق snapshots.

---

## Task — Staff & Employees Final Closure: جولة الإغلاق النهائية لقسم الموظفين
- الحالة: **مكتملة ✅ ومعتمدة ومدموجة** (جولة إغلاق صغيرة→متوسطة — ليست مرحلة)
- اعتماد المالك (حرفيًا): «اعتماد نهائي — PR #32 ✅ … أوافق رسميًا على دمج PR #32» — 2026-07-11
- الدمج: ‏PR #32 ‏squash إلى `main@7ffc06f` («Staff and employees final closure (#32)»)
- التاريخ: 2026-07-11 · الأساس: `origin/main@f05842d` · منفذة بأمر المالك بعد اعتماد البحث المرجعي والـ Audit والخطة (بقراراتها الـ 17) — **صفر Migrations**، والبنية القائمة (User/HotelMembership/HotelPermissionGrant) مصدر الحقيقة بلا أي إعادة بناء.
- **1) منع تعديل الصلاحيات ذاتيًا:** حارس صريح في `set_staff_permissions` **قبل** أي منطق — المستخدم لا يعدل منح عضويته بنفسه إطلاقًا (إضافة/إزالة/استبدال/إعادة إرسال نفس المجموعة) → **`cannot_edit_own_permissions` (403)**؛ المدير كذلك؛ مفروض خلفيًا (خدمة) ومحجوب في مصفوفة الواجهة. قاعدة «لا تمنح إلا ما تملك» باقية بلا توسيع.
- **2) حارس الوردية المفتوحة:** `deactivate_staff_member` (بقفل صف العضوية) يرفض التعطيل إن وُجدت وردية مفتوحة للموظف في الفندق → **`staff_has_open_shift` (409)** برسالة توجيه؛ **لا إغلاق/نقل تلقائي** ولا أي منطق ورديات جديد؛ التفعيل يعيد نفس العضوية بنفس PK.
- **3) ترقية `staff→manager` / تنزيل `manager→staff`:** خدمتان تحت **`staff.manage_managers`**؛ الفاعل يجب أن يكون **مديرًا فعليًا** لا مجرد حامل للصلاحية (**`not_a_manager` 403**)؛ لا ذاتيًا (**`self_action_blocked`**)؛ داخل معاملة بقفل؛ الترقية **تحفظ المنح الفردية خاملة** (المدير يرث الكل) وتعود عند التنزيل؛ لا تغيير تلقائي لـ`is_primary_manager`.
- **4) حماية آخر مدير والمدير الأساسي:** دالة موحّدة `_assert_manager_removable` تُقفل **كل عضويات المديرين النشطة للفندق** ثم تعيد العدّ داخل المعاملة — تحمي عبر **التعطيل والتنزيل والحذف** الثلاثة؛ آخر مدير نشط → **`last_manager_protected` (409)**، والمدير الأساسي → **`primary_manager_protected` (409)** (يمنع سباق المديرَين المتزامن).
- **5) تغيير البريد:** خدمة `change_staff_email` تحت **`staff.change_email`**؛ لا ذاتيًا؛ التطبيع + الفرادة العالمية؛ **يُرفض إن كان للمستخدم أي عضوية أخرى نشطة أو تاريخية أو دور منصة** (الهوية عابرة للمستأجرين) → **`cross_tenant_identity` (409)** بلا بناء مسار مركزي؛ حدث بالقديم→الجديد؛ لا رسالة/دعوة.
- **6) الحذف المحكوم:** `delete_staff_membership` — خريطة أثر مركزية `_TRACE_MAP` تفحص **كل حقول الفاعل عبر 9 تطبيقات** (ورديات/تسليم/إغلاق يومي/حجوزات/إقامات/فوليو/قيود/دفعات/مصروفات/فواتير/طلبات وطاولات/تدبير وصيانة ومفقودات/سجلات حالات الغرف/الأنشطة actor+target/created_by/updated_by/منح) — أي أثر → **`staff_has_trace` (409)** والتعطيل هو البديل؛ العضوية النظيفة تُحذف هي ومنحها؛ **حذف User** فقط إن كان نظيفًا وحيدًا (لا عضوية أخرى، لا دور منصة، لا أثر عالمي) — بلا cascade عمياء. أحداث الحذف بلا `target_user` (الاسم في النص) فيبقى سجل التدقيق صالحًا (وActivityEvent.actor/target_user كلاهما SET_NULL).
- **7) الصلاحيات الثلاث الجديدة:** **`staff.delete`** / **`staff.change_email`** / **`staff.manage_managers`** — مستقلة (لا تُستعمل `staff.update`)، والمدير يرثها؛ لا تغيير في route guards ولا إعادة تصميم RBAC.
- **8) سجل النشاط (10–11 حدثًا):** ‏created / linked_existing_user / **updated بالحقول المتغيرة قديم→جديد ولا حدث دون تغيير** / deactivated (بالسبب) / reactivated / **email_changed** / **promoted_to_manager** / **demoted_to_staff** / permissions_updated (بالمضاف والمسحوب) / **membership_deleted** / **user_deleted** — بالفندق والعضوية والفاعل والوقت والقيم؛ **لا تسجيل لأي كلمة مرور**.
- **9) سريان السحب/التعطيل فورًا:** مُثبت أن فحص الصلاحية يضرب قاعدة البيانات في كل طلب (لا كاش/توكن) — منح ثم سحب ثم طلب HTTP تالٍ = 403 فورًا، وتعطيل ثم طلب تالٍ = `membership_inactive` فورًا، بلا إعادة دخول؛ لم يُبنَ أي Token Revocation جديد.
- **10) Reset Password** بلا توسيع: محمي بصلاحيته، لا يعرض ولا يسجل كلمة المرور، لا يتجاوز العزل.
- **Migrations: صفر** (`makemigrations --check` = No changes detected).
- اختبارات: ‏backend من 947 إلى **972** (+25 جديدًا؛ مع تكييف 4 اختبارات قائمة للسلوك الجديد لحجب التعديل الذاتي)؛ ‏i18n ‏2309→**2337×3** بتكافؤ تام (+28).
- سبر حي معزول بفندقين+ثالث مؤقتين (HTTP عبر APIClient بتراجع ذري): **‏22/22** سيناريو تغطي أوامر السبر الـ 27 — عزل كامل مثبت.

---

## Task — Expenses Final Closure: جولة الإغلاق النهائية لقسم المصروفات
- الحالة: **مكتملة ✅ ومعتمدة ومدموجة** (جولة إغلاق صغيرة→متوسطة — ليست مرحلة)
- اعتماد المالك (حرفيًا): «اعتماد نهائي — PR #31 ✅ … أوافق رسميًا على دمج PR #31» — 2026-07-11
- الدمج: ‏PR #31 ‏squash إلى `main@7a55da6` («Expenses final closure (#31)»)
- التاريخ: 2026-07-11 · الأساس: `origin/main@308d821` · منفذة بأمر المالك بعد اعتماد البحث المرجعي والـ Audit والخطة (بقراراتها الـ 12 + تثبيت تسجيل التعديلات الوصفية بالقيم القديمة والجديدة)
- **1) سد ثغرة P0 (تحرير المال الصامت):** مسار PATCH صار عبر خدمة مركزية جديدة `update_expense` تقبل **الحقول الوصفية الأربعة فقط** (الوصف/الملاحظات/المرجع/المستفيد) **وداخل يوم السند المفتوح حصرًا**؛ أي حقل مالي (المبلغ/الفئة/الطريقة/العملة/التواريخ/الوردية/الحالة/العكس) يُرفض في الخدمة والـ Serializer والـ View والواجهة — وقيد DB (`expense_amount_sign`) يمنع أي سالب خارج صفوف العكس.
- **2) ‏business_date:** حقل جديد يُختم حصريًا من `get_business_date(hotel)`؛ ‏`paid_at` صار **وقت تنفيذ فقط** يختمه الخادم بـ now؛ إرسال `paid_at`/`business_date`/`currency` من العميل = 400؛ وسقط `timezone.localdate` من منطق المصروفات.
- **3) نافذة الـ Void:** داخل يوم السند المفتوح فقط بسبب إلزامي وقفل صف (**`void_window_closed` 409** بعدها) — توحيد كامل مع فلسفة ‎#29.
- **4) القيد العكسي:** خدمة `reverse_expense` — نسخة **كاملة سالبة** برقم EXP جديد مرتبطة بالأصل (`reverses` self-FK محمي)، بنفس الفئة/الطريقة/العملة/المستفيد/المرجع، بتاريخ اليوم الجاري، **تدخل درج وردية منفّذها تلقائيًا** (العكس النقدي يعيد النقد للدرج بالمعادلة القائمة دون أي تعديل)؛ عكس سارٍ واحد لكل أصل (قيد فريد شرطي + ‏409 `expense_already_reversed`)؛ لا عكس لسند voided ولا لعكس؛ والنافذة المفتوحة ترفضه بتوجيه للـ void (**`void_window_open`**)؛ إبطال العكس بيومه يفتح عكسًا مصححًا.
- **5) الصلاحية الجديدة:** **`expenses.reverse`** مستقلة (المدير يرثها) — ‏endpoint ‏`expenses/{id}/reverse/` محمي خلفيًا وواجهيًا.
- **6) العملة:** عملة الفندق حصرًا (تُفرض في الخدمة وحُذفت من كل مدخلات الـ API) — لا Data Migration للقديم.
- **7) الاشتقاقات على business_date** بسقوط آمن للقديم (‏fallback من paid_at بتوقيت الفندق): ‏`unassigned_movements` و`build_daily_snapshot` (**للدفعات والمصروفات معًا** — استكمال توحيد ‎#29) وFinance Overview وفلاتر تاريخ القائمة.
- **8) سجل النشاط (4 أحداث):** ‏expense.created / **expense.updated بالقيم القديمة والجديدة للحقول المتغيرة فقط ولا حدث دون تغيير فعلي** (تثبيت المالك) / expense.voided بالسبب / expense.reversed بمرجعي الأصل والعكس.
- **9) سند الصرف المطبوع:** أُثري (عبر الـ serializer وواجهة الطباعة القائمة) بـ: الحالة، ‏business_date، وقت التنفيذ، الوردية، سبب الإبطال ومن أبطل، و«عكسٌ لـ…»/«عُكس بـ…» — مستند تشغيلي فقط، قابل لإعادة الطباعة.
- **10) الواجهة (وظيفي فقط):** زر تعديل بحواره الرباعي + زر عكس بحوار سبب مع سطر إيضاحي + شارات `↩` للربط + حجب الأزرار الثلاثة بالصلاحيات (`useCan`) + عمود التاريخ على business_date — والخادم وحده يقرر النوافذ.
- ‏Migration واحدة (`finance.0005`): الحقلان + القيدان + الفهرس + ‏Backfill آمن من `paid_at` بتوقيت الفندق — **فحص قبلي: صفر مبالغ سالبة قديمة**؛ لا حذف بيانات.
- اختبارات: ‏backend من 919 إلى **947** (+28: ختم 4، حراس PATCH ‏6، نافذة void ‏4، عكس 10، اشتقاقات وطباعة ونشاط 3، صمامات DB ‏2 — دون كسر القائمة)؛ ‏i18n ‏2292→**2309×3** بتكافؤ تام (+17).
- سبر حي معزول بفندقين مؤقتين: **‏24/24** سيناريو (تغطي أوامر السبر الـ 17 كاملة) — تنظيف اعتمادي كامل مثبت بتطابق snapshots.

---

## Task — Restaurant & Café Final Closure: جولة الإغلاق النهائية لقسم المطعم والكافتيريا
- الحالة: **مكتملة ✅ ومعتمدة ومدموجة** (جولة إغلاق متوسطة→كبيرة — ليست مرحلة)
- اعتماد المالك (حرفيًا): «اعتماد نهائي — PR #30 ✅ … أوافق رسميًا على دمج PR #30» — 2026-07-11
- الدمج: ‏PR #30 ‏squash إلى `main@c896c2a` («Restaurant and cafe final closure (#30)»)
- التاريخ: 2026-07-11 · الأساس: `origin/main@cfc7d85` · منفذة بأمر المالك بعد اعتماد البحث المرجعي والـ Audit والخطة (بقراريها النهائيين: لا مساس بفوليو مغلق إطلاقًا — تصحيحات الدفع المباشر تؤجل للمالية، وإبقاء `source`/`item_type` قديمين مهجورين بلا حذف)
- **1) سد ثغرة الإقامة (P0):** حارس **`stay_not_in_house` (409)** في ثلاث نقاط: إنشاء Room Order وربط الإقامة بطلب طاولة (تحقق عند الإنشاء)، الترحيل للفوليو (**تحقق نهائي تحت قفل صف الإقامة**)، و`finance.ensure_stay_folio` نفسها — فلا فوليو تشغيلي جديد لإقامة منتهية أبدًا. ‏Check-in وقراءة/طباعة الفوليو بعد المغادرة والتصحيحات المالية القائمة **بلا أي تغيير** (اختبارات انحدار صريحة).
- **2) المنفذان الثابتان + مفتاحا التفعيل:** ‏restaurant/cafe قيمتان ثابتتان (لا نموذج Outlets ديناميكيًا)؛ ‏`restaurant_enabled`/`cafe_enabled` في إعدادات الفندق (افتراضيًا مفعلان — الفنادق الحالية بلا تغيير)؛ التعطيل يمنع الجديد فقط (طلبات/طاولات/تصنيفات/أصناف — **`outlet_disabled` 409**) ويخفي مسارات الإنشاء في الواجهة، والقديم يبقى مقروءًا وللتقارير.
- **3) الطاولات البسيطة:** نموذج `RestaurantTable` (منفذ ثابت/رقم/اسم اختياري/سعة/حالة يدوية available|out_of_service بسبب إلزامي للإخراج)؛ **‏occupied مشتقة حصرًا** من وجود طلب مفتوح؛ فرادة الرقم داخل الفندق والمنفذ؛ **طلب مفتوح واحد لكل طاولة** بقفل صف + قيد DB جزئي (`unique_open_order_per_table`)؛ خارج-الخدمة لا تقبل طلبًا، والمشغولة لا تخرج من الخدمة؛ المستعملة لا تحذف (`resource_in_use`). لا حجز/دمج/تقسيم/نقل/مخطط صالة.
- **4) نوعا الطلب:** ‏`order_type` ‏room (إقامة in_house إلزامية والغرفة مشتقة، بلا طاولة) | ‏table (طاولة إلزامية من نفس المنفذ متاحة وغير مشغولة، إقامة مقيمة اختيارية، **عميل خارجي = `customer_name` نص حر فقط** بلا Guest/Customer/Account)؛ الشكل كله (النوع/المنفذ/الطاولة/الإقامة) **ثابت بعد الإنشاء** (رفض 400)؛ عناصر الطلب يجب أن تنتمي لمنفذه (**`outlet_mismatch`**).
- **5) الدفع المباشر عبر فوليو عابر:** خدمة `settle_order_direct` — دورة ذرية واحدة بخدمات المالية المركزية حصرًا: فوليو عابر (بلا إقامة/حجز، عملة الفندق) ← قيد واحد بإجمالي الطلب ← دفعة كاملة (**تدخل درج وردية المنفّذ تلقائيًا**) ← رصيد صفر ← **إغلاق فوري نهائي** ← ربط الطلب (folio/charge/payment/receipt)؛ أي فشل = ‏rollback كامل (مثبت بmock)؛ **لا مساس بالفوليو المغلق بعدها إطلاقًا** — التصحيح اللاحق من قسم المالية، وحوار تأكيد إلزامي في الواجهة يشرح ذلك (×3 لغات).
- **6) XOR صارم:** ‏settlement ‏(unsettled→direct|folio، لا رجوع) منفصل عن حالة التشغيل («تم التسليم» ≠ «تم الدفع»)؛ مفروض في الخدمة (بقفل صف الطلب) وفي DB (**`service_order_settlement_xor` CheckConstraint**) وفي الواجهة؛ لا جمع/جزئي/تقسيم/إعادة تسوية/ترحيل مدفوع/دفع مرحّل (**`order_already_settled`/`order_already_posted` 409**).
- **7) إلغاء عنصر منفرد:** قبل التسوية فقط، بسبب إلزامي، السطر لا يحذف (‏snapshot السعر والكمية باقٍ + الفاعل والوقت)، يستبعد من الإجماليات ومن KOT، **آخر عنصر فعال لا يلغى** (`last_active_item_not_cancellable` — بديله إلغاء الطلب)؛ لا إلغاء كمية جزئيًا.
- **8) إلغاء الطلب:** قبل التسوية فقط بسبب إلزامي، نهائي، يحرر الطاولة اشتقاقيًا؛ المسوّى يصحح ماليًا فقط (قواعد ‎#29).
- **9) business_date:** يُختم على الطلب من `get_business_date(hotel)` (إرساله من العميل = 400)؛ ‏Overview وفلاتر التاريخ عليه (بسقوط آمن للقديم)؛ فهرس `(hotel, business_date)`؛ سقط `timezone.localdate` من منطق الطلبات.
- **10) الحقول القديمة:** ‏`source` و`item_type` **بقيا مهجورين للقراءة فقط** (معلّمين Legacy/Deprecated في الكود، خارج الـ serializers الكتابية والواجهة والتقارير الجديدة)؛ ‏Backfill آمن نسخ معناهما إلى `order_type`/`outlet` (وoutlet التصنيف من أصنافه) و`settlement` للمرحّل القديم و`business_date` من `ordered_at` بتوقيت الفندق — **بلا حذف أي بيانات؛ هجرة تنظيف مستقلة لاحقًا بعد التحقق**.
- **11) الصلاحيتان الجديدتان:** **`services.tables_manage`** (إدارة الطاولات وحالتها) و**`service_orders.settle_direct`** (الدفع المباشر) — مفروضتان خلفيًا وواجهيًا؛ إلغاء العنصر تحت `service_orders.update` والترحيل تحت `post_to_folio` كما هما.
- **12) سجل النشاط (9 أحداث):** ‏created / items_updated / item_cancelled / cancelled / status_changed / paid_direct / posted_to_folio + ‏table.out_of_service / back_in_service — بالفاعل والسبب والمبلغ والطريقة حيث يلزم.
- **13) الطباعة (النظام القائم فقط):** ‏KOT (عناصر وملاحظات تحضير **بلا أسعار**) + ‏Guest Check (أسعار وضرائب وإجماليات قبل التسوية) عبر `variant` على endpoint التذكرة، وإيصال الدفع المباشر بإيصال المالية القائم — كلها قابلة لإعادة الطباعة؛ الملغى لا يطبع؛ لا KDS.
- ‏Migrations (2): ‏`services.0002` (الطاولات + 12 حقلًا + 3 قيود + فهرس + ‏Backfill يدوي آمن) و`hotels.0004` (المفتاحان) — فحص اتساق قبلي سليم، لا حذف Legacy.
- اختبارات: ‏backend من 872 إلى **919** (+47: حراس 5، منافذ 4، طاولات 10، طلب طاولة 8، دفع مباشر 11، إلغاء عنصر 5، تواريخ 3، نشاط 1 — مع تكييف 46 القائمة واختبار تقارير)؛ ‏i18n ‏2237→**2292×3** بتكافؤ تام (+65، −10 مهجورة).
- سبر حي معزول بفندقين مؤقتين (أحدهما كافتيريا-فقط): **‏39/39** سيناريو تغطي أوامر المالك الـ 26 (الطباعة عبر اختبارات الـ API) — تنظيف اعتمادي كامل مثبت بتطابق snapshots.

---

## Task — Guest Folio Final Closure: جولة الإغلاق الأساسية لقسم فوليو النزيل
- الحالة: **مكتملة ✅ ومعتمدة ومدموجة** (جولة إغلاق أساسية متوسطة — ليست مرحلة)
- اعتماد المالك (حرفيًا): «تمت مراجعة التقرير بالكامل، وأوافق على اعتماد PR #29 من الناحية المعمارية والتنفيذية» — 2026-07-11
- الدمج: ‏PR #29 ‏squash إلى `main@8add680` («Guest folio final closure (#29)»)
- التاريخ: 2026-07-11 · الأساس: `origin/main@2e48ba3` · منفذة بأمر المالك بعد اعتماد البحث المرجعي وتقرير الـ Audit وخطة الإغلاق (بقراراتها الـ 16 + البند الاختياري B15 + حارس Reservation Folio)
- **1) حماية الفوليو المغلق/المبطل (read-only كامل):** كل الـ void (قيد/دفعة/فوليو) والقيود الجديدة والتسويات والفواتير الجديدة وحتى تعديل الملاحظات تُرفض على فوليو غير مفتوح (**`folio_closed` 409**)؛ لا `closed → open` بأي مسار؛ `void_folio` يمر فقط على فوليو مفتوح **خالٍ تمامًا** من أي سطر مالي أو فاتورة غير مبطلة (**`folio_has_postings` 409**) — فوليو الأسطر الملغاة مساره الإغلاق لا الإبطال (يحفظ التاريخ).
- **2) حارس يوم العمل + توحيد business_date:** كل قيد جديد يُختم بـ`get_business_date(hotel)` (سقط `timezone.localdate` من المنطق المالي)، والدفعة تُختم بحقل جديد `business_date` + `paid_at=now`؛ **التأريخ الرجعي والمستقبلي مستحيل بنيويًا** (حذف `charge_date`/`paid_at`/`currency` من الـ API؛ إرسالها = 400)؛ `ensure_business_day_open` يشمل الآن القيود اليدوية أيضًا.
- **3) قاعدة Void الموحدة:** «داخل يوم السجل المفتوح = Void بسبب إلزامي؛ بعده = قيد معاكس مرتبط» — نافذتان لا تتداخلان: void خارج يوم السجل → **`void_window_closed` 409**، وتسوية/عكس ونافذة الـ void ما زالت مفتوحة → **`void_window_open` 409** (يقلب قرار Phase 12 القديم بالسماح للـ void بعد الإقفال — موثق).
- **4) Adjustment مركزي:** `adjust_charge` = قيد معاكس **كامل** (سالب إجمالي الأصل) بنوع adjustment مرتبط بالأصل (`adjusts` self-FK) بسبب إلزامي بتاريخ اليوم الجاري؛ الأصل لا يُمس؛ **تسوية واحدة سارية لكل أصل** (قيد فريد شرطي — إلغاء التسوية بيومها يفتح تصحيحها)؛ لا تسوية لتسوية؛ **`charge_already_adjusted` 409**.
- **5) Payment Reversal مركزي:** `reverse_payment` = دفعة معاكسة **كاملة** (سالبة، بإيصال RCP جديد ونفس الطريقة) مرتبطة بالأصل (`reverses` self-FK) بسبب إلزامي، تدخل درج وردية المنفّذ تلقائيًا؛ عكس واحد سارٍ لكل دفعة (**`payment_already_reversed` 409**)؛ لا عكس لعكس؛ التسمية التشغيلية «عكس دفعة» لا Refund.
- **6) صلاحيتان جديدتان:** **`finance.adjust`** و**`finance.payment_reverse`** — مستقلتان عن charge_create/payment_void، والمدير يرثهما كالمعتاد؛ ‏endpoints: ‏`charges/{id}/adjust/` و`payments/{id}/reverse/`.
- **7) فوليو مفتوح واحد لكل Stay:** الحارس انتقل للخدمة المركزية بقفل صف الإقامة + **قيد DB فريد شرطي** (`unique_open_folio_per_stay`) كصمام سباق؛ خدمة **`ensure_stay_folio`** ‏get-or-create idempotent؛ فوليوهات مغلقة متعددة تاريخيًا مسموحة.
- **8) إنشاء الفوليو آليًا عند Check-in:** `CheckInService` يستدعي `ensure_stay_folio` داخل نفس المعاملة (ربط Stay+Guest+Reservation، اسم النزيل، عملة الفندق، **بلا أي قيد/دفعة/deposit**)؛ فشل الفوليو = ‏rollback كامل للدخول (مثبت باختبار mock وسبر حي)؛ ترحيل المطعم يستعمل الخدمة نفسها.
- **9) العملة:** عملة الفوليو = عملة الفندق حصرًا (من `HotelSettings.default_currency`، تُفرض في الخدمة وحُذف الحقل من الواجهة)؛ لا Data Migration للقديم؛ ‏Overview لا يجمع عملات: الرصيد المعلق لعملة الفندق فقط + عدّاد `foreign_currency_folios` وتنبيه في الواجهة.
- **10) سجل النشاط المالي الكامل:** 11 حدثًا عبر `record_activity` القائم: ‏folio.created/closed/voided، ‏charge.posted/voided/adjusted، ‏payment.recorded/voided/reversed، ‏invoice.issued/voided — بالمبلغ والفاعل والسبب للعمليات الحساسة (طباعة الكشف قراءة صرفة فلا تُسجَّل — متسق مع نمط الإيصال/الفاتورة).
- **11) كشف الفوليو (Statement):** ‏`GET folios/{id}/statement/` بنمط مستندات الطباعة القائم + زر في الواجهة لكل الحالات (المغلق/المبطل **إعادة طباعة فقط**): رأس الفندق، النزيل/الإقامة/الغرفة/التواريخ، القيود كاملة (الملغي معلَّم والتسويات بروابط أصلها)، الدفعات (والعكوس بروابطها)، الإجماليات والرصيد — ليس فاتورة ضريبية ولا محرك PDF جديد.
- **12) تفريع تصحيح المطعم المرحّل:** قيد الطلب يخضع للقاعدة الموحدة (يومه مفتوح = void؛ بعده = adjustment مرتبط)؛ الطلب يبقى posted بمرجعه `posted_charge` ولا يُرحَّل ثانية أبدًا؛ `_resolve_folio` توحّد على `ensure_stay_folio`.
- **13) فاتورة سارية واحدة (B15):** `issue_invoice` يرفض إصدارًا جديدًا مع وجود فاتورة issued غير مبطلة للفوليو (**`active_invoice_exists` 409**)؛ بعد الإبطال يُصدر من جديد؛ بلا Credit Note وبلا ترقيم قانوني.
- **14) حارس Reservation Folio:** فوليو بحجز بلا إقامة يُرفض (**`reservation_folio_not_supported` 400**)؛ الربط التوثيقي بالحجز مع الإقامة باقٍ؛ بلا Deposit Ledger وبلا تحويل حجز→إقامة.
- ‏Migration واحدة (`finance.0004`): ‏`adjusts` + ‏`reverses` (self-FK محمية) + ‏`business_date` (nullable للقديم) + 3 قيود فريدة شرطية + فهرس — **فحص قرائي قبلي: صفر إقامات بأكثر من فوليو مفتوح**؛ لا لمس بيانات، لا تعديل عملات قديمة.
- اختبارات: ‏backend من 812 إلى **872** (+60: فوليو 54 + دخول/فوليو 4 + تصحيح مطعم 2؛ مع تكييف اختبارات قائمة لإعادة استخدام فوليو الدخول الآلي)؛ ‏i18n ‏2213→**2237×3** بتكافؤ تام؛ ‏Expenses لم تُمس (تبعيتها موثقة لقسم المصروفات).
- سبر حي معزول بفندقين مؤقتين (تنظيف اعتمادي كامل مُثبت بصفر مخلفات): **39/39** سيناريو، تشمل الدورة كاملة والتصحيحين والمغادرة والعملة والفاتورة السارية وحارس الحجز والـrollback والإغلاق اليومي.

---

## Task — Housekeeping Final Closure: جولة الإغلاق النهائية لقسم التدبير الفندقي
- الحالة: **مكتملة ✅ ومعتمدة ومدموجة** (جولة إغلاق صغيرة→متوسطة — ليست مرحلة)
- اعتماد المالك (حرفيًا): «اعتماد نهائي — PR #28 ✅ … أوافق على دمج PR #28» — 2026-07-11
- الدمج: ‏PR #28 ‏squash إلى `main@3dfcf6e` («Housekeeping final closure (#28)»)
- التاريخ: 2026-07-11 · الأساس: `origin/main@0c53490` · منفذة بأمر المالك بعد اعتماد البحث المرجعي وتقرير الـ Audit
- **1) سقف مهمة نشطة واحدة للغرفة:** فحص مركزي في `create_housekeeping_task` بقفل صف الغرفة — خطأ جديد **`duplicate_active_task` (409)**؛ المسارات التلقائية (Check-out ونقل الغرفة) تمرر `on_active="skip"` فلا تفشل معاملاتها أبدًا؛ ‏idempotency مهمة المغادرة القائمة محفوظة؛ ‏awaiting_inspection تُحسب نشطة.
- **2) تدفق العامل والمشرف:** تعيين لموظف محدد + إعادة تعيين + **إزالة تعيين تعيد المهمة pending وتمسح المعين** — بأنشطة (assigned/reassigned/unassigned)؛ فلتر **`mine=true`** في الـ API وواجهة «مهامي فقط»؛ منتقي موظفين في الواجهة (بسقوط آمن لخيار «أنا» لمن لا يرى قائمة الموظفين).
- **3) الفحص الاختياري:** إعداد **`housekeeping_inspection_required`** لكل فندق (افتراضيًا معطل — سلوك الفنادق الحالية بلا تغيير)؛ عند التفعيل: إكمال العامل → **`awaiting_inspection`** والغرفة لا تتحرر؛ **اعتماد** (صلاحية جديدة **`housekeeping.inspect`**) → completed + الغرفة available بعد حارس الصيانة (**مانع الصيانة سيد القرار** — مثبت)؛ **رفض بسبب إلزامي** (`inspection_reason_required`) → الغرفة dirty والمهمة in_progress والعامل يكملها من جديد، وسجل الرفض محفوظ في status log.
- **4) الأولوية:** ترتيب severity صريح (urgent→high→normal→low) عبر `PRIORITY_RANK` + ‏annotation في الـ API (لا ترتيب نصي)؛ تعديل الأولوية بعد الإنشاء من الواجهة مع نشاط **قديم→جديد**.
- **5) سجل النشاط:** عبر `record_activity` القائم: إنشاء/تعيين/إعادة/إزالة/بدء/إكمال/awaiting/اعتماد/رفض/إلغاء/تغيير أولوية + إغلاق وإلغاء الصيانة + إنشاء/تسليم/إتلاف المفقودات + **كل تغيير حالة غرفة عبر الخدمة المركزية** (`room.status_changed`).
- **6) صلاحيات الواجهة:** كل أزرار تبويبات العمليات الأربعة محجوبة بـ`can()` (إنشاء/تعيين/بدء/إكمال/إلغاء/فحص/صيانة/مفقودات).
- **7) الفلاتر:** كشف الموجود أصلًا: نوع المهمة + المعيَّن + التاريخ + «مهامي» + ترتيب بالأولوية (بلا API جديد).
- **8) ‏RoomBoardTab:** إصلاح التحميل الجزئي بحلقة صفحات داخلية (بسقف صارم) بدل الصفحة الأولى الصامتة — بلا رفع حد الـ Pagination العام.
- **9) وصول اليوم غير الجاهز:** ‏endpoint قرائي أدنى **`housekeeping/arrivals-not-ready`** (مشتق لحظيًا: حجوزات اليوم المؤكدة المثبتة على غرف غير available أو مشغولة — لا تخزين) + شارة وعدّاد وفلتر في اللوحة. **لا تجاوز للجاهزية ولا مساس بالتوفر.**
- **10) توثيق الدلالة:** ‏`out_of_service` = حجب كامل عن البيع والحجز والدخول؛ ‏`maintenance` = حجب مؤقت مرتبط ببلاغ صيانة — موثقة في الكود والنصوص، **بلا أي تغيير في enum أو NON_BOOKABLE أو التوفر**.
- ‏Migrations (2 مبررتان): ‏`hotels.0003` (حقل الإعداد، افتراضي false) + ‏`operations.0002` (توسيع status لاستيعاب awaiting_inspection — البيانات القائمة سليمة). اختبارات operations: ‏71→**94** (+23). لم يُنفَّذ Stay-over ولا DND ولا أي بند مؤجل.
- سبر حي معزول (نُظف بالكامل والإعداد والغرف مستعادة snapshot): **19/19** سيناريو خدمي + سبر HTTP للصلاحيات.

---

## Task — Guests Profiles Final Closure: جولة الإغلاق الأساسية لقسم النزلاء
- الحالة: **مكتملة ✅ ومعتمدة ومدموجة** (جولة إغلاق أساسية — ليست مرحلة)
- اعتماد المالك (حرفيًا): «اعتماد نهائي — PR #26 ✅ … أوافق على دمج PR #26» — 2026-07-10
- الدمج: ‏PR #26 ‏squash إلى `main@b51034e` («Guests profiles final closure (#26)»)
- التاريخ: 2026-07-10 · الأساس: `origin/main@c963ff0` · منفذة بأمر المالك بعد تقرير مراجعة القسم الشاملة
- **المفهوم:** قسم النزلاء سجل مركزي تاريخي — عرض وتصحيح بيانات وتصنيف فقط؛ لا تعديل ولا حذف للتاريخ التشغيلي (حجوزات/إقامات/فوليو).
- **1) ملف النزيل المركزي:** ‏endpoint قرائي `guests/<pk>/profile/` + نافذة ملف كاملة: الهوية + الحالات (نشط/VIP/محظور مع السبب للمصرح) + إحصاءات **مشتقة لا مخزنة** (إقامات فعلية غير ملغاة، ليالٍ، أول/آخر إقامة، متكرر = أكثر من إقامة فعلية، مقيم الآن = وجود in_house مع الغرفة) + الإقامة/الحجز/الفوليو الحاليين + منشئ/آخر معدل.
- **2) سجل الإقامات:** داخل الملف، **قراءة فقط**: الغرفة/النوع/التواريخ/الليالي/الحالة/الفوليو مع شارة «حالية»، وروابط انتقال للحجز (find بالرقم) والاستقبال والمالية **محكومة بصلاحيات الأقسام المستهدفة** — لا أي تعديل تشغيلي من القسم.
- **3) قاعدة الظهور:** ‏endpoint جديد `guests/directory/` للقسم يعرض فقط من له إقامة فعلية واحدة على الأقل (annotation + prefetch واحد — بلا N+1)، بينما `guests/` القديم بقي كما هو **لمنتقيات الحجز والدخول** (لا كسر للـ Wizard/الفوري/Check-in). صف الدليل: الاسم+الشارات/الهاتف/الجنسية/مقيم الآن+الغرفة/عدد الإقامات+الليالي+جديد-متكرر/آخر إقامة/فتح الملف.
- **4) ‏VIP:** حقول `is_vip/vip_marked_at/vip_marked_by` + تفعيل/إلغاء عبر خدمة مركزية بصلاحية جديدة **`guests.mark_vip`**، شارة في الدليل والملف **وبطاقات المقيمين في الاستقبال** (`primary_guest_is_vip` على serializer الإقامة) — بلا مستويات/نقاط/مال.
- **5) الحظر الفندقي:** ‏`is_blocked/block_reason/blocked_at/blocked_by` على النزيل + سجل **`GuestBlockLog`** غير قابل للفقدان (السبب القديم يبقى بعد الفك)، سبب إلزامي (`block_reason_required`)، صلاحية جديدة **`guests.block`**، والعزل فندقي بالبناء (النموذج per-hotel — لا حظر عالمي).
- **6) أثر الحظر:** خطأ نطاق **`guest_blocked` (409)** في الخدمات المركزية: ‏`create_reservation` (مطابقة snapshot بالوثيقة أو الهاتف — إنشاء Guest جديد لنفس الشخص لا يتجاوز الحظر؛ يشمل الكونسول والفوري والموقع العام) + ‏`CheckInService` (الرئيسي والمرافقون)؛ السبب لا يُكشف في الخطأ، لا تجاوز للمدير، والفك يعيد كل شيء.
- **7) تشديد الحذف:** أي أثر تشغيلي (إقامة **أو فوليو أو مفقودات**) ⇒ تعطيل فقط عبر خدمة مركزية؛ الحذف الصلب للملف النظيف فقط؛ الاستجابة تفرّق **`deleted`/`deactivated`** والواجهة تعرض الرسالة الصادقة.
- **8) حماية الوثائق:** صلاحية جديدة **`guests.view_sensitive_data`** + دالة إخفاء مركزية (`••••` + آخر 4) مطبقة **في طبقة الـ API** (القائمة والدليل والملف)؛ القيم المقنعة مرفوضة في الكتابة (لا تلوث الملف)، والـ Wizard لا ينسخ رقمًا مقنعًا إلى snapshot الحجز؛ الفرادة وsnapshots القديمة بلا تغيير.
- **9) التعديل:** بقي محدودًا بالبيانات الأساسية عبر `guests.update`، وأزرار الواجهة كلها (إضافة/تعديل/حذف/VIP/حظر) محجوبة بالصلاحيات.
- **10) النشاط:** عبر `record_activity` القائم: إنشاء/تعديل (بقيم قديمة→جديدة للاسم والهاتف، والوثيقة **مقنعة**)/VIP وإلغاؤه/حظر/فك/تعطيل/حذف — بلا أي رقم وثيقة كامل في النصوص.
- ‏migration واحدة مبررة (حقول VIP والحظر + جدول سجل الحظر؛ قيم افتراضية آمنة — كل النزلاء الحاليين غير VIP وغير محظورين). ترجمات **+36 ×3** (تكافؤ **2190**) + كودا خطأ معرّبان. اختبارات: ‏+39 (‏guests ‏17→56) — الحزمة الكاملة **789/789**.
- سبر حي معزول (نُظف بالكامل والبيانات الأصلية مطابقة snapshot): ‏16/16 خدميًا + ‏HTTP: الإخفاء «••••7766» لحامل view فقط والرقم كامل للمدير، السبب مخفي، ‏403 على vip/block/unblock/delete، والدليل يعد النزلاء ذوي الإقامات فقط.

---

## Task — Front Desk / Stays Final Closure: جولة الإغلاق الأساسية لقسم الدخول والمغادرة
- الحالة: **مكتملة ✅ ومعتمدة ومدموجة** (جولة إغلاق أساسية — ليست مرحلة)
- اعتماد المالك (حرفيًا): «اعتماد نهائي — PR #25 ✅ … أوافق على دمج PR #25» — 2026-07-10
- الدمج: ‏PR #25 ‏squash إلى `main@7ecff87` («Front desk and stays final closure (#25)»)
- التاريخ: 2026-07-10 · الأساس: `origin/main@bf6e148` · منفذة بأمر المالك بعد تقرير مراجعة القسم الشاملة
- **المبدأ المعماري:** بعد تسجيل الدخول تصبح الإقامة (Stay) مصدر الحقيقة التشغيلي — التمديد والتقصير والنقل تُنفَّذ من قسم Stays حصراً عبر خدمات مركزية تعيد فحص التوفر بمحرك التوفر القائم وتُبقي الحجز موحدًا معها (بلا منطق موازٍ).
- **1) سقف كمية سطر الحجز عند الدخول:** استثناء جديد `reservation_line_full` (‏409) — قفل صف السطر (`select_for_update`) ثم عدّ الإقامات غير الملغاة ورفض تجاوز الكمية (سطر ×1 = إقامة واحدة؛ ×2 = اثنتان)؛ نفس السقف على مستوى الحجز عند الدخول بلا سطر. آمن ضد التزامن ولا يعتمد على الواجهة. **6 اختبارات** بينها عزل الفنادق.
- **2) حارس تاريخ الوصول:** ‏`arrival_date_in_future` (‏409 مع تاريخ الوصول وتاريخ العمل) — الدخول قبل تاريخ الوصول ممنوع بحسب `get_business_date(hotel)` (لا `timezone.localdate()` في قواعد Front Desk)؛ الوصول بيوم الحجز مسموح أيًا كانت الساعة، والحجوزات المتأخرة تدخل ما دامت صالحة. **5 اختبارات** بينها فندق بمنطقة زمنية (Kiritimati UTC+14) يختلف تاريخه عن الخادم بلحظة مجمدة.
- **3) تمديد الإقامة:** ‏`ExtendStayService` — ‏in_house فقط، تاريخ أحدث فقط، Transaction + أقفال (الإقامة والغرفة)، فحص النافذة المضافة عبر `ensure_can_book` باستثناء حجز الإقامة نفسه (حجز تالٍ = `room_assignment_conflict`؛ نوع ممتلئ = `no_availability`؛ صيانة/خارج خدمة = `room_not_ready`)، وعند النجاح يتحدث تاريخا الإقامة والحجز معًا (نمو فقط) بلا مساس بالوصول ولا رسوم جديدة. سجل StayStatusLog (قديم→جديد + السبب + المستخدم) + نشاط `stay.extended`.
- **4) تقصير الإقامة:** ‏`ShortenStayService` — تاريخ أسبق فقط، ولا قبل تاريخ العمل ولا على/قبل تاريخ الدخول؛ يحرر المخزون بتقليص نهاية الحجز عبر قاعدة موحدة تحمي الأسطر غير المُدخلة بعد؛ **لا يحذف رسومًا ولا يسجل خروجًا**. سجل + نشاط `stay.shortened`.
- **5) نقل الغرفة:** ‏`RoomMoveService` — سبب إلزامي، in_house فقط، قفل الغرفتين بترتيب ثابت (pk)، شروط الغرفة الجديدة: نفس الفندق + نشطة + `available` (لا dirty/cleaning) + غير مشغولة (إشغال مشتق) + سعة كافية + حرة حتى نهاية الإقامة (`ensure_can_book`)؛ عند النجاح: تحديث غرفة الإقامة، السطر يتبع النزيل (إعادة تثبيت) لتوحيد التوفر، القديمة → dirty + مهمة تنظيف تلقائية (بلا مفتاح الإقامة كي لا تُسقط مهمة الخروج لاحقًا)، وسجل نقل كامل (قديمة/جديدة/وقت/مستخدم/سبب) + نشاط `stay.room_moved`. **11 اختبارًا**.
- **6) ربط المغادرة بالفوليو:** عبر `folio_balance` و`close_folio` المركزيتين فقط — رصيد غير صفري (موجب أو سالب) = **رفض** بـ`folio_balance_outstanding` (409) دون أي تغيير؛ رصيد صفري = خروج + إغلاق الفوليو مركزيًا؛ لا فوليو = خروج طبيعي بلا إنشاء. endpoint قرائي جديد `folio-summary` يغذي واجهة المغادرة (الرصيد + الحالة + زر فتح الفوليو + تعطيل التأكيد)، ونص «الفوترة مرحلة لاحقة» القديم صُحح. **5 اختبارات**.
- **7) المغادرة المبكرة:** الخروج قبل الموعد = مغادرة مبكرة — **السبب إلزامي** (`early_departure_reason_required`)، الطبيعية بلا سبب؛ نهاية الحجز تنكمش لتحرير الليالي المتبقية (بأرضية ليلة واحدة ومع حماية الأسطر المعلقة)، الرسوم لا تُمس، وتُسجل «early departure» في السجل والنشاط. قفل صف الإقامة أضيف للخروج (يمنع سباق الخروج المزدوج). **6 اختبارات**.
- **8) توحيد business_date:** وصول اليوم ومغادرة اليوم وكل مقارنات «اليوم» التشغيلية في القسم صارت على `get_business_date(hotel)` — بلا تغيير في منطقه المركزي.
- **9) الصلاحيات:** ثلاثة أكواد جديدة بنمط الـ registry: `stays.extend` / `stays.shorten` / `stays.move_room` (المدير يرثها تلقائيًا) — محمية في الباك إند لكل endpoint، والواجهة تخفي أزرار الدخول/الخروج/التمديد/التقصير/النقل عمن لا يملكها (`useHotelAccess.can`). بلا أي تغيير في route guards أو بنية RBAC.
- **10) قائمة الغرف الصالحة:** ‏endpoint جديد `check-in-rooms` (والمرشحات `move-candidates` للنقل) يعيد الغرف القابلة فعليًا (متاحة يدويًا + غير مشغولة اشتقاقيًا + غير مثبتة لحجز آخر متداخل + سعة كافية للنقل) — مودال الدخول صار يستهلكه بدل قائمة الحالة اليدوية، وفحص الباك إند النهائي باقٍ كما هو.
- ترجمات: **+35 ×3** (تكافؤ **2154**) + تصحيح نص financeNote + 5 أكواد أخطاء جديدة معرّبة في errors.ts. **صفر migrations** (كل الجديد خدمات/فحوص/endpoints). اختبارات stays: ‏28 → **82**.
- سبر حي (بيئة معزولة مؤقتة نُظفت بالكامل والبيانات الأصلية أثبتت مطابقتها snapshot قبل/بعد): 12/12 سيناريو خدمي + صلاحيات HTTP (محدود 403 على النقاط الست الجديدة كلها ولا يحمل أي stays.*، والمدير مصرَّح).

---

## Task — Reservations Final Closure: جولة إغلاق قسم الحجوزات
- الحالة: **مكتملة ✅ ومعتمدة ومدموجة** (جولة تصحيح محدودة — ليست مرحلة)
- اعتماد المالك (حرفيًا): «اعتماد نهائي — PR #24 ✅ … أوافق على دمج PR #24» — 2026-07-10
- الدمج: ‏PR #24 ‏squash إلى `main@0a65e25` («Reservations final closure safeguards (#24)»)
- التاريخ: 2026-07-10 · الأساس: `origin/main@4c0a33c` · منفذة بأمر المالك بعد تقرير المراجعة الشاملة
- **1) حارس ما بعد تسجيل الدخول:** استثناء domain جديد `reservation_has_active_stay` (‏409): مع إقامة `in_house` مرتبطة بالحجز — **تعديل التواريخ أو سطور الغرف مرفوض** و**الإلغاء مرفوض** (الإقامة مصدر الحقيقة؛ التغيير من الدخول والمغادرة)، بينما الحقول الآمنة (ملاحظات/طلبات/بيانات اتصال) تمر؛ بلا أي مزامنة خفية أو rollback أو مساس بالإقامة. حقل قرائي جديد `has_in_house_stay` في serializer الحجز يقود الواجهة: الـ Wizard يجمّد حقول التواريخ والغرف ويستثنيها من الـ PATCH مع تنبيه ورابط للدخول والمغادرة، وزر الإلغاء يختفي من الصف والتفاصيل مع رسالة تفسيرية. **6 اختبارات** + **سبر حي**: تعديل تاريخ 409 · تغيير غرفة 409 · إلغاء 409 · ملاحظات 200 · الإقامة سليمة · تعديل ما قبل الدخول حر 200.
- **2) إبراز طلبات إلغاء الموقع:** ‏`request_public_cancellation` صار يسجل نشاطًا داخليًا عبر `record_activity` القائم (رقم الحجز + الضيف + المصدر + السبب؛ **idempotent** — لا تكرار؛ لا رسائل خارجية ولا نظام جديد) + فلتر قرائي `cancel_requested=true` (معلّق = ما زال held/confirmed — المُلغى اعتُبر معالجًا فتزول الشارة تلقائيًا بلا حالة جديدة) + في القائمة **شارة «طلب إلغاء من الموقع»** ومفتاح فلتر «طلبات إلغاء معلّقة»، والتفاصيل تعرضه تنبيهًا كما كانت. **3 اختبارات** + **سبر حي**: نشاط مسجل بمحتواه، تكرار = 1، الفلتر يعيد المعلقة، بيانات الشارة على الصف.
- **3) تنظيف الحقل الميت:** أزيل `no_show_reason` من نوع الواجهة ومن عرض التفاصيل (لا يكتبه أي مسار في الباك إند) — بلا migration وبلا مساس بالحالات.
- ترجمات: +4 ×3 برسائل المالك الحرفية — تكافؤ **2119**. ‏Backend read-only خالص (استثناء + حقل محسوب + فلتر + نشاط) — صفر هجرات.

## Task — Reservations Section Reorganization: إعادة تنظيم قسم الحجوزات
- الحالة: **مكتملة ✅ بكاملها** (الأساس #22 + جولة الـ Wizard والحجز الفوري #23)
- اعتماد جولة الـ Wizard (حرفيًا): «اعتمد PR #23 رسميًا ✅ — المراجعة التشغيلية النهائية مقبولة» — 2026-07-10
- دمج الجولة: ‏PR #23 ‏squash إلى `main@53f92b0` («Reservations wizard and immediate check-in flow (#23)»)
- **جولة Wizard والحجز الفوري (بأمر المالك، 2026-07-10):** وصل الأمر «داخل نفس PR #22» بعد دمجه بأمر المالك — فنُفذت على فرع معاد الإنشاء بنفس الاسم من `origin/main@5f052a4` **بدون فتح PR** (السابقة المعتمدة في مهمة الغرف). المنفَّذ:
  - **حذف فلترَي «أُنشئ من/إلى» من الواجهة** (يبقيان مدعومَين في الـ API للاستخدام الداخلي وتبويب «حجوزات اليوم» كما هو) — الفلاتر الظاهرة: بحث/حالة/مصدر/نوع/وصول من-إلى/مسح.
  - **فورم الحجز صار Wizard بأربع خطوات**: بيانات الضيف (مع **اختيار ضيف مسجّل** يملأ البيانات تلقائيًا — قائمة الضيوف القائمة) ← المرافقون (بالغون/أطفال — نموذج الحجز يدعم الأعداد؛ الأسماء تُلحق عند الدخول بالتصميم) ← الوثائق (حقلا النوع والرقم القائمان في الحجز؛ لا رفع ملفات لأن النظام لا يدعمه) ← بيانات الحجز (النوع/التواريخ/الليالي/الغرف مع التوفر الحي/المصدر/الحالة/الملاحظات + المراجعة). تحقق مرحلي (لا تقدم بلا اسم الضيف) + تحقق نهائي (تواريخ صحيحة، خروج بعد وصول، سطر غرفة، وغرفة محددة إلزامية للفوري) + أزرار: إلغاء/السابق/التالي/«حفظ الحجز» للمستقبلي و**«حفظ وتسجيل الدخول»** للفوري.
  - **الحجز الفوري = إنشاء + دخول بالخدمة القائمة حرفيًا**: تاريخ الوصول يُقرأ من **`business_date` الجديد في overview الحجوزات** (إضافة قرائية من سطر — ساعة العميل تختلف عن الفندق، مثبت سابقًا)؛ الحالة مؤكدة (شرط الدخول القائم)؛ الدورة: ضيف مسجّل/جديد (‏Guest عبر API الضيوف — الدخول يتطلب ملف ضيف كما في front-desk) ← `createReservation` ← ‏**`POST /stays/check-in` القائم بكل تحققاته** (غرفة متاحة، لا إشغال، لا تعارض، صلاحية `stays.check_in`) ← شاشة نجاح (الرقم/الضيف/الغرفة/التواريخ + فتح الحجز/عرض الإقامة/فوليو النزيل). **فشل الدخول بعد نجاح الحجز لا يُخفى**: شاشة «تم إنشاء الحجز، لكن فشل تسجيل الدخول» + السبب + فتح الحجز + الذهاب إلى الدخول والمغادرة — بلا rollback خطير. **كرت «حجز فوري» معطّل بتلميح لمن لا يملك `stays.check_in`.**
  - **الحجز المستقبلي**: كما اعتُمد — بلا دخول ولا stay، بشاشة نجاح (الرقم/الضيف/التواريخ + فتح الحجز).
  - **‏Deep links**: ‏`action=new` (مع غرفة أو بدونها) يفتح **نافذة الاختيار دائمًا** والغرفة تُمرَّر للـ Wizard أيًا كان الكرت (خيار «أسلم UX» الذي نص عليه المالك)؛ ‏`action=find` كما هو؛ ‏useQuickAction بلا مساس. التبويبات الثمانية بلا تغيير — لا تبويبات مقيمين/دخول/خروج.
  - **إثبات تشغيلي حي عبر الـ APIs بنفس ترتيب الـ Wizard**: ضيف جديد 201 ← حجز فوري R00005 على الغرفة 105 بتاريخ العمل 201 ← ‏**check-in القائم 201** والغرفة ظهرت «مشغولة» باسم النزيل على لوحة الغرف ← ومسار الفشل: حجز R00006 على غرفة متسخة ثم دخول **مرفوض 409 room_not_ready** والحجز باقٍ (حالة شاشة الفشل الجزئي بالضبط).
  - ترجمات: +18 ×3 + تحديث وصف الفوري («إنشاء حجز وتسجيل دخول النزيل مباشرة من الاستقبال.») — تكافؤ **2115**. ‏Backend: سطر `business_date` في overview + اختبار (‏reservations ‏81/81).
- الحالة السابقة: **مكتملة ✅** (مهمة — ليست مرحلة)
- اعتماد المالك (حرفيًا): «اعتمد PR #22 رسميًا ✅ — المراجعة النهائية مقبولة» وفق القاعدة: الحجوزات = إدارة الحجز قبل أن يتحول إلى إقامة فعلية — 2026-07-10
- الدمج: ‏PR #22 ‏squash إلى `main@1165ce5` («Reservations section reorganization (#22)»)
- التاريخ: 2026-07-10 · الأساس: `origin/main@b90d955`
- **المبدأ:** الحجوزات = إدارة الحجز قبل أن يصير إقامة — لا «مقيم/دخول/خروج» كتصنيفات داخل القسم؛ تلك للـ front-desk وحده. حذف تبويب «النظرة العامة» القديم (كان يخلط بعدادات وصول/مغادرة) لصالح كروت العدادات الجديدة.
- **الصفحة:** ترويسة بعنوان «الحجوزات» + الوصف المعتمد + زر أساسي **«حجز جديد»** (‏reservations.create) يفتح فورم الإنشاء القائم مباشرة (إشارة للمكوّن الحي) · **7 كروت عدادات قابلة للنقر** (كل الحجوزات/اليوم/الموقع/مستقبلية/بانتظار التأكيد/مؤكدة/ملغاة-مرفوضة — العدادات من 7 استعلامات `page_size=1` فتطابق التبويبات دائمًا) · **8 تبويبات** = المشاهد السبعة + «التوفر» القائم (أُبقي لأنه من صميم الحجز) — والقائمة تبقى حية عبر التبديل (تُخفى تحت التوفر) فلا تفقد فلاترها، وزر التحديث العام يعيد كل شيء.
- **تعريفات المشاهد (كلها شرائح مفلترة من نفس القائمة):** اليوم = **أُنشئ اليوم بتاريخ العمل الفندقي** (نافذة اليوم بتوقيت الفندق — لا «وصول اليوم») · الموقع = `source=public_website` مع إبراز المصدر · مستقبلية = `check_in > تاريخ العمل` (حصرًا) · بانتظار التأكيد = `held` · مؤكدة = `confirmed` · ملغاة/مرفوضة = `cancelled+expired` (تبقى سجلًا). التبويبات المرتبطة بحالة تخفي فلتر الحالة (وتبويب الموقع يخفي فلتر المصدر) منعًا للتناقض.
- **Backend (‏read-only محدود — مبرر):** فلاتر جديدة على list القائمة فقط: ‏`source` و`statuses` (‏CSV مُتحقق) و`created_today` و`upcoming` (كلاهما بتاريخ العمل الفندقي وبتوقيت الفندق — يستحيل حسابهما بدقة من العميل) و`created_from/to` و`room`، وتوسيع البحث ليشمل **رقم الغرفة** المسندة — **6 اختبارات جديدة**، **الحزمة كاملة 693/693 ✅**، صفر هجرات، صفر تغيير في منطق الحجز/الدخول/المغادرة.
- **القائمة:** أعمدة جديدة: **المصدر** (‏badge، الموقع العام أزرق) و**تاريخ الإنشاء** و**عدد النزلاء** + **زر نسخ رقم الحجز**؛ فلاتر جديدة: المصدر + تاريخا الإنشاء من/إلى + **مسح الفلاتر**، والبحث الحر يشمل رقم الغرفة؛ والأزرار مقيدة بالأكواد الفعلية: تأكيد ← `reservations.confirm`، إلغاء ← `reservations.cancel`، تعديل ← `reservations.update` (زر تعديل أضيف للصف). **لا «رفض» مستقل في المنطق الحالي** — الإلغاء بسبب يغطي رفض حجوزات الموقع (موثق، لا اختراع حالة). لا مال على الحجز (‏Phase 6 بلا مال — الفوليو في المالية) فلا أعمدة مالية. وفي تفاصيل الحجز المؤكد **رابط خفيف** «الدخول والمغادرة» (لا تنفيذ دخول/خروج من القسم).
- **Deep links:** ‏`?tab=reservations` و`overview` القديمان يُطبَّعان إلى «كل الحجوزات» فلا ينكسر أي رابط قائم؛ ‏`action=new&room=&room_type=` و`action=find&q=` تعمل كما اعتُمدت (‏useQuickAction بلا مساس) — مؤكد بجلسة مصادَقة 200.
- ترجمات: قسم `reservations.views` (18 ×3 بنصوص المالك) — تكافؤ **2091** — مع إعادة استخدام تسميات المصدر القائمة. الكروت تعيد استخدام أنماط `board-stat` المركزية.
- **تحقق:** ‏lint ✅ tsc ✅ build ✅ · ‏Django check ✅ · لا هجرات ✅ · ‏693/693 ✅ · حيًا: الصفحة والروابط العميقة الأربعة وfront-desk وrooms كلها 200، والعدادات السبعة تعيد أرقامًا صحيحة من بيانات الفندق التجريبي.
- **تصحيح UX (بأمر المالك):** (أ) **‏Empty State بلا زر** — أيقونة + «لا توجد حجوزات» + «لا توجد حجوزات مطابقة للفلاتر الحالية.» فقط (زر الإنشاء واحد أعلى الصفحة). (ب) **زر «حجز جديد» يفتح Modal اختيار النوع** بكرتين كبيرين قابلَي النقر (جنبًا إلى جنب على desktop وفوق بعض على mobile — ليس dropdown): **حجز فوري** («إنشاء حجز يبدأ من اليوم أو الآن.» — يفتح الفورم بنمط instant وتاريخ وصول اليوم، **يبقى حجزًا** ولا check-in تلقائي، والباك إند يعيد التحقق بتاريخ العمل الفندقي) و**حجز مستقبلي** («إنشاء حجز بتاريخ وصول قادم.» — يفتح الفورم بنمط future وتاريخ الوصول متروك للمستخدم). (ج) **‏Deep links بالأفضلية التي حددها المالك**: ‏`action=new` مع `room/room_type` يفتح **الفورم مباشرة** بالغرفة المحددة؛ وبدونهما يفتح **نافذة الاختيار** — ‏useQuickAction بلا مساس. ترجمات +6 ×3 (تكافؤ **2097**) وCSS مركزي `choice-cards/card`. صفر Backend هذه الجولة.

## Task — Rooms & Floors Operational Board: لوحة الغرف والطوابق التشغيلية
- الحالة: **مكتملة ✅** (مهمة — ليست مرحلة؛ الأساس #20 + جولتا تصحيح UX ‏#21)
- اعتماد جولات التصحيح (حرفيًا): «اعتمد PR #21 رسميًا ✅ — المراجعة النهائية مقبولة» — 2026-07-10
- دمج التصحيحات: ‏PR #21 ‏squash إلى `main@7c03f18` («Rooms UX corrections - simplified forms and operational board cleanup (#21)»)
- **الجولة النهائية — تبسيط وتوزيع (بأمر المالك، 2026-07-10):** (أ) **أزيل شريط الأزرار الإداري كليًا من أعلى الصفحة** — النظرة التشغيلية صارت عرض/فهم/تصفية فقط تبدأ بالكروت ثم الطوابق ثم الفلاتر ثم الغرف؛ ووزعت الإجراءات على أقسامها: أنواع الغرف في تبويبها، الطوابق في تبويبها، وإضافة غرفة + **إضافة مجموعة غرف** زران في رأس تبويب الغرف. (ب) **زر تحديث عام في التوب بار** (أيقونة صغيرة): آلية حدث مركزي خفيفة (`globalRefresh`) — صفحة الغرف تستمع وتعيد جلب كل شيء، والصفحات غير المستمعة لا تتأثر ولا يوجد reload كامل. (ج) **توحيد «غرفة»** — أزيلت كل صياغات «وحدة» من الواجهة («إضافة غرفة جديدة»، «رقم الغرفة»، «نوع الغرفة»، «حالة الغرفة»، «إضافة مجموعة غرف»). (د) **تبويب الطوابق تبسّط لإعداد واحد**: «عدد الطوابق» + «حفظ عدد الطوابق» + chips الطوابق الحالية — الزيادة تنشئ «الطابق N» تلقائيًا (حقول الاسم/الرقم/الترتيب تُملأ خلفيًا وأخفيت عن المستخدم)، والتقليل يُمنع تحت أعلى طابق مستخدم برسالة المالك الحرفية ويحذف الفارغة الذيلية فقط؛ **حُذف فورم إضافة طابق التفصيلي**. (هـ) **فورم نوع الغرفة تبسّط**: اسم + **سعة واحدة** (تزامن base/max خلفيًا) + **«سعر الليلة»** + **مميزات multi-select** من كتالوج 24 ميزة معتمدة (chips تُضاف وتحذف، بلا تكرار، تُخزن كمفاتيح ثابتة في حقل `amenities` القائم) + وصف + نشط + **«عرض على الموقع العام» مفعّل افتراضيًا** — وأخفيت: الرمز (يولَّد تلقائيًا لأنه إلزامي)، السعة الأساسية/القصوى، «السعر المرجعي»، نوع السرير، والاسم/السعر/الوصف العام (تُحفظ قيمها القديمة بلا مساس). والنوع المعطّل لا يُعرض عند إنشاء غرف (يبقى ظاهرًا في غرفه القديمة وفي تعديلها). (و) **فورم الغرفة تبسّط لخمسة حقول**: رقم/طابق/نوع (المفعّلة فقط)/حالة/نشط — حُذف الاسم المعروض وكل تلميحات السعر/السعة (تأتي من النوع)، ومثله فورم المجموعة (زالت أساس الاسم وعرض النوع). (ز) كرت الغرفة والـ Drawer: أزيل الاسم المعروض، والسعر صار «سعر الليلة». ترجمات: **+40 ×3 (منها 24 ميزة) + 7 تحديثات صياغة** — تكافؤ **2073**. حُذف مودالا الإدارة السابقان (`RoomTypesManagerModal`/`FloorsManagerModal`) بعد انتقال منطقهما للتبويبات. **صفر Backend** وendpoint اللوحة بلا مساس. فحوص: lint ✅ tsc ✅ build ✅ · الصفحات الست 200 · زر التحديث في HTML المقدَّم · لوحة المنصة سليمة.
- اعتماد الأساس (حرفيًا): «اعتمد PR #20 رسميًا ✅» بعد مراجعة تشغيلية ببيانات حية غطت الحالات الست — دُمج squash إلى `main@4f13bd6` وتوثيق `9b6d636` (2026-07-10)
- **جولة تصحيح UX/الفورمات (بأمر المالك، 2026-07-10):** وصل أمر التصحيح «داخل نفس PR #20» بعد أن كان الـ PR مدموجًا ومحذوف الفرع بأمر المالك نفسه — فنُفذت الجولة على فرع بنفس الاسم من آخر `origin/main@9b6d636` **بدون فتح PR** (التزامًا بـ«لا تفتح PR جديدًا») وقرار الوعاء للمالك. المنفَّذ:
  - **شريط أدوات أعلى الصفحة** (فوق التبويبات، ظاهر دائمًا، يلتف على الموبايل): أنواع الغرف · تعديل الطوابق · إضافة غرفة/وحدة · إضافة نطاق غرف/وحدات · تحديث — كلٌّ بصلاحيته (إدارة/إنشاء/عرض)، وزر التحديث يعيد تركيب التبويب النشط فيعاد جلب كل شيء.
  - **مودال أنواع الغرف**: وصف تعريفي + فورم إضافة (اسم بمثال «جناح ملكي»، رمز مُقترح تلقائيًا لأنه إلزامي في الـ API، سعة افتراضية/قصوى، سعر أساسي، وصف، مفعّل) يحدّث القائمة فورًا دون إغلاق + قائمة الأنواع بصفوف (سعة/سعر/حالة) مع تعديل داخلي وتفعيل/تعطيل، والحذف يظهر **فقط للأنواع غير المستخدمة** (والباك إند يرفض حذف المستخدم أصلًا — تُعرض رسالة واضحة مع نصيحة التعطيل).
  - **مودال تعديل الطوابق**: عدد الطوابق المفعّلة + chips الطوابق الحالية (بعدد غرف كلٍّ) + النص المساعد؛ الزيادة تنشئ الطوابق الناقصة تلقائيًا («الطابق N» بلغة الجلسة)، والتقليل يحذف **الفارغة الذيلية فقط** ويُمنع تحت أعلى طابق مستخدم برسالة صريحة (وحرس الباك إند القائم دفاع إضافي). لا «عدد طوابق» مخزّن — واجهة ذكية فوق Floor records القائمة، صفر Backend.
  - **مودال غرفة موحّد** (`RoomFormModal` يخدم الإضافة من الشريط + تعديل من الكرت + تبويب الغرف): رقم بمثال، طابق، نوع، اسم عرض، **سعة وسعر/ليلة يُقرآن تلقائيًا من النوع** (لا حقول غرفة خلفية لهما — لم تُختلق)، **حالة أولية/معدّلة عبر endpoint الحالة المضبوط** (بصلاحية rooms.status_update، مع حقل الملاحظة الإلزامي للصيانة/خارج الخدمة)، مفعّل. التجهيزات/العملة/الملاحظات/الظهور العام **لم تُعرض** لأن الغرفة لا تملك حقولها في الباك إند (التجهيزات والظهور العام على مستوى النوع من Phase 15) — موثق للمالك.
  - **مودال النطاق**: أضيفت الحالة الأولية (بنفس المسار المضبوط) وعرض السعة/السعر من النوع وplaceholders (101/110) ورسالة «أدخل نطاقًا صحيحًا من/إلى» + جلب أرقام الغرف القائمة ذاتيًا للمعاينة/كشف المكرر (يُستثنى تلقائيًا دون إيقاف الباقي) وسقف 100.
  - **كروت الإحصائيات**: 8 كروت (إجمالي primary، متاحة للحجز، مشغولة الآن، محجوزة، تحتاج تنظيف، تحت التنظيف، **صيانة/خارج الخدمة موحّد**، تحتاج انتباه) بعنوان ورقم كبير **ووصف صغير تحت الرقم**، والموحّد يفلتر الحالتين معًا.
  - **نظرة عامة على الطوابق**: قسم معنون، كرت الطابق أضيف له **Progress bar** لنسبة الإتاحة، وأعمدة الشبكة صارت بمقاس محدد (18–26rem) فلا يتمدد كرت الطابق الوحيد شريطًا فارغًا ضخمًا.
  - **كرت الغرفة**: الرقم داخل **badge واضح** والنوع تحته مباشرة، سطر تشغيلي بالصيغ المعتمدة («جاهزة للاستقبال الآن»، «بحاجة للتنظيف قبل الإتاحة»، «قيد التنظيف الآن»، «غير متاحة للاستخدام حاليًا»، «مقيم: فلان»، «قادم: فلان»)، والأزرار: **إجراء الحالة الأساسي + عرض (Drawer) + تعديل** (‏rooms.update) — تغيير الحالة والإجراءات الكاملة في الـ Drawer، والمشغولة إجراؤها «تفاصيل الإشغال».
  - ترجمات: **58 مفتاحًا جديدًا ×3 + تحديث قيمتين** («اكتب للبحث...»، «إضافة نطاق غرف / وحدات») — تكافؤ **2033**. ‏CSS مركزي (toolbar/captions/progress bar/number badge/type rows). **صفر Backend** وendpoint اللوحة بلا مساس.
  - فحوص: ‏lint ✅ tsc ✅ build ✅ · صفحات rooms/reservations/operations/front-desk/finance كلها 200 بجلسة مصادَقة · الـ CSS المقدَّم يحمل الأنماط الخمسة الجديدة · ‏board API ‏200.
- **الهدف:** تحويل `/hotel/rooms` من إدارة غرف عادية إلى لوحة تشغيلية يعرف منها المدير فورًا وضع كل غرفة وطابق وما الإجراء التالي.
- **Backend (‏read-only محدود — مبرر):** ‏endpoint واحد جديد `GET /rooms/operational-board/` (صلاحية `rooms.view`، صفر كتابة، صفر هجرات) لأن الدقة تستحيل من الواجهة وحدها: ‏listRooms مقسّمة صفحات، وربط الإقامة الحالية والحجز القادم لكل غرفة عبر ثلاث قوائم مقسّمة = N+1 وبيانات ناقصة. يعيد DTO كاملًا: ‏summary (9 عدادات) + ‏floors (عدادات لكل طابق + نسبة إتاحة) + ‏rooms (كل غرفة مع `display_status` المحسوب و`current_stay` و`next_reservation`). **‏5 اختبارات جديدة** (صلاحية، عزل مستأجرين، أولويات العرض، تخطي حجز الإقامة الحالية، استثناء المؤرشف من العدادات) — **الحزمة كاملة 687/687 ✅**.
- **‏display_status (للعرض فقط — لا يُخزَّن أبدًا):** بأولوية المالك حرفيًا: ‏archived ← out_of_service ← maintenance ← cleaning ← dirty ← **occupied** (‏Stay ‏in_house) ← **reserved** (‏Reservation ‏held/confirmed بسطر غرفة و`check_out ≥` تاريخ العمل الفندقي) ← available. ‏`operational_status` المخزّن يُعاد كما هو بجانبه.
- **الواجهة (مكوّنات صغيرة):** ‏`RoomOperationalBoard` (منسق + فلاتر) · `RoomSummaryCards` (9 كروت قابلة للنقر + الإجمالي؛ نقرة تفلتر ونقرة ثانية تلغي؛ «تحتاج انتباه» = dirty+cleaning+maintenance+out_of_service) · `FloorSummaryCards` (كرت لكل طابق بعداداته ونسبة الإتاحة؛ نقر = فلتر طابق يتراكب مع فلتر الحالة، مع chip واضح) · شريط فلاتر (بحث حر برقم/اسم الغرفة/النوع/اسم النزيل الحالي أو القادم/رقم الحجز + طابق + نوع + حالة + إظهار المؤرشف + **مسح الفلاتر**) · `RoomFloorGrid` (عناوين طوابق بملخص مصغر + Grid كروت) · `RoomOperationalCard` (رقم كبير + badge بلون هادئ + النوع/الطابق/السعة/السعر المرجعي + سطر تشغيلي حسب الحالة: المقيم وتاريخ مغادرته، أو الحجز القادم بتواريخه ورقمه، أو ملاحظة الحالة وآخر تغيير) · `RoomDetailsDrawer` (تفاصيل كاملة + كل الإجراءات) · `RoomStatusModal` (استُخرج مركزيًا من RoomsTab ويشترك فيه التبويبان).
- **أزرار الكرت حسب الحالة والصلاحيات (نفس قاعدة any-of):** متاحة ← «حجز جديد» يفتح فورم الحجز القائم **مع الغرفة ونوعها محددين مسبقًا** (`?action=new&room=&room_type=`)؛ محجوزة ← «عرض الحجز» (`?action=find&q=رقم الحجز` — بحث فوري) و«دخول النزيل» (وصول اليوم)؛ مشغولة ← عرض الإقامة/فوليو النزيل/مغادرة؛ dirty ← «إنشاء مهمة تنظيف» بفورم التنظيف القائم **والغرفة محددة**؛ cleaning ← عرض مهمة التنظيف؛ maintenance ← عرض/إنشاء طلب صيانة بالغرفة المحددة؛ out_of_service/archived ← تفاصيل + تغيير الحالة (`rooms.status_update`). ‏`useQuickAction` وُسّع ليمرر باراميترات الغرفة وينظفها من الرابط بعد الاستهلاك، وفورمات الحجز/التنظيف/الصيانة القائمة صارت تقبل قيمة أولية — **صفر منطق أعمال جديد**.
- **إضافة نطاق غرف:** ‏`BulkRoomCreateModal` (زر بصلاحية `rooms.create`): طابق + نوع + من/إلى + بادئة اختيارية + أساس اسم عرض اختياري + تفعيل — **معاينة صريحة** قبل الحفظ تعرض الأرقام وتكشف **المكررة وتستثنيها** (من بيانات اللوحة حيًا)، سقف 100 غرفة، تنفيذ متسلسل عبر `createRoom` القائم (لا bulk endpoint — غير ضروري) مع تعطيل الأزرار ونص تقدم i/N وملخص نهائي صادق (أُنشئ X، فشل Y مع السبب). إضافة الغرفة المفردة القائمة في تبويب «الغرف» بلا مساس.
- **التبويبات:** نظرة تشغيلية (الجديدة، أولى) · الطوابق · أنواع الغرف · الغرف — القائمة كلها محفوظة؛ ‏OverviewTab القديم حُذف (حلّت اللوحة مكانه)؛ تنبيه «أضف طابقًا ونوع غرفة قبل إنشاء الغرف» عند غياب الأساسات.
- **ترجمات:** قسم `rooms.board` كامل (**57 مفتاحًا ×3** بنصوص المالك حرفيًا حيث وردت — تكافؤ **1975**). ‏CSS مركزي بنمط `--room-accent` القائم وألوان هادئة (أخضر/أزرق/برتقالي/أحمر/رمادي) وresponsive (كروت عمود واحد على الموبايل، فلاتر تلتف).
- **تحقق:** ‏lint ✅ tsc ✅ build ✅ · ‏Django check ✅ · ‏makemigrations --check ✅ (لا هجرات) · ‏**687/687** ✅ · وبجلسة مصادَقة: الـ endpoint يعيد بيانات Ops Hotel 10 الحقيقية (101 dirty، ‏102 available، العدادات صحيحة) وكل مسارات الروابط العميقة الستة 200 والـ CSS المقدَّم يحمل أنماط اللوحة ولوحة المنصة سليمة.

## Task — App Shell: ثلاث بطاقات مستقلة (سايدبار / توب بار / محتوى)
- الحالة: **مكتملة ✅** (مهمة — ليست مرحلة)
- اعتماد المالك (حرفيًا): «اعتمد PR #19 رسميًا ✅» — 2026-07-10
- الدمج: ‏PR #19 ‏squash إلى `main@e76ce1c` («App Shell task - three-card shell, hotel topbar identity, dashboard & quick operations (#19)»)
- التاريخ: 2026-07-09 → 2026-07-10 · الأساس: `origin/main@dddc09a`
...
- **تثبيت ترتيب لوحة التحكم: الشريط في القمة (بطلب المالك):** شريط العمليات المباشرة صار **أول محتوى `/hotel`** — فوق كروت الإحصائيات مباشرة بعد ترويسة الصفحة، فالترتيب النهائي: إجراءات سريعة ← إحصائيات اليوم ← الوصول السريع. لا شيء آخر تغيّر: نفس القسم المعنون والأزرار التسعة المباشرة وصلاحياتها وآلية `useQuickAction`، والمكوّن ما زال مستورَدًا في صفحة لوحة التحكم وحدها (لا يظهر في أي صفحة أخرى — مؤكد بنيويًا وبجلسات مصادَقة)، والتوب بار نظيف بصفٍّ واحد.
- **تصحيح مكان شريط العمليات: لوحة التحكم فقط (بطلب المالك):** الشريط **أُزيل من التوب بار كليًا** — التوب بار عاد بطاقة صفٍّ واحد نظيفة (هوية الفندق + badge الاشتراك + الجرس + اللغة + المستخدم + الخروج الأحمر فقط؛ حُذفت بنية الصفين من الـ markup والـ CSS). وانتقل الشريط إلى **صفحة `/hotel` وحدها** كقسم معنون «إجراءات سريعة» بين كروت الإحصائيات وكروت الاختصارات — والدليل البنيوي أن المكوّن مستورَد في صفحة لوحة التحكم فقط فلا يمكن ظهوره في أي صفحة أخرى (مؤكد أيضًا بجلسات مصادَقة على rooms/reservations/finance). **منطق الأزرار التسعة لم يُمس**: عمليات مباشرة بنفس `useQuickAction` والفورمات القائمة والصلاحيات نفسها لكل زر. تكيّف العرض للسياق الجديد: أزرار أوضح قليلًا (font-size-sm بظل خفيف)، ‏desktop سطر يلتف بأناقة، وعلى ≤640px **شبكة عمودين دائمة** بدل زر الطي (لم يعد ضروريًا خارج التوب بار) — صفر overflow. صفر Backend وصفر مفاتيح ترجمة جديدة.
- **تصحيح شريط الإجراءات: عمليات مباشرة تفتح الفورم فورًا (بطلب المالك):** الأزرار لم تعد روابط أقسام — كل زر يفتح **فورم الإنشاء القائم مباشرة** عبر `?action=new` بآلية مركزية جديدة `useQuickAction` (تستهلك الباراميتر **مرة واحدة**: تفتح المودال القائم ثم تمسح `action` من الرابط بـ router.replace مع بقاء `?tab=` — فإغلاق الفورم أو التحديث أو الرجوع لا يعيد فتحه، والنقرة التالية تعمل من جديد). وُصلت بستة فورمات قائمة حرفيًا (صفر منطق جديد): **حجز جديد** → مودال إنشاء الحجز في تبويب الحجوزات · **إضافة نزيل** → GuestModal · **إضافة مصروف** → ExpenseModal · **طلب خدمة** → مودال طلب الخدمة · **مهمة تنظيف** ومودال المهمة · **طلب صيانة** ومودال الطلب. **دخول/مغادرة نزيل** يفتحان سطح الاختيار القائم (وصول/مغادرة اليوم — إجراء الدخول/المغادرة على كل بطاقة، إذ العملية مرتبطة بحجز/إقامة بالتصميم)، و**تسجيل دفعة** يفتح قائمة الفوليوهات (فورم الدفعة داخل الفوليو حصرًا — تصميم Phase 8، كما سمح المالك). الشريط صار **9 أزرار** بالترتيب اليومي المعتمد (أزيلت فوليو/تقرير/إعدادات — تنقّل صرف يغطيه السايدبار ولوحة التحكم). ولإتمام الآلية: صفحة الحجوزات صارت تقرأ `?tab=` والمضيفات الأربع (حجوزات/استقبال/عمليات/خدمات) **تتابع تغيّر الرابط وهي مفتوحة** — نقر زر سريع وأنت داخل الصفحة نفسها يبدّل التبويب ويفتح الفورم. صفر Backend، صفر مفاتيح ترجمة جديدة (القائمة ضمن مفاتيح الجولة السابقة)، والصلاحيات كما هي لكل زر.
- **شريط إجراءات سريعة في التوب بار (بطلب المالك):** **سطر ثانٍ داخل بطاقة التوب بار نفسها** (قشرة الفندق فقط — فاصل ناعم تحت صف الهوية/الأدوات، البطاقات الثلاث كما هي): 12 زرًا صغيرًا أنيقًا (أيقونة + تسمية) بترتيب الأهمية التشغيلية — **حجز جديد** (primary بتدرج العلامة) ثم دخول نزيل، مغادرة نزيل، إضافة نزيل، فوليو النزيل، تسجيل دفعة، إضافة مصروف، طلب خدمة، مهمة تنظيف، صيانة، تقرير سريع، الإعدادات (soft هادئة). **كل زر يحترم صلاحيات Phase 11 بنفس قاعدة any-of للسايدبار** (المدير يرى الكل؛ لا زر بلا صلاحيته؛ لا شيء أثناء تحميل السياق). الروابط لصفحات قائمة فقط — ولتوصيل الأزرار بدقة أُضيفت **قراءة `?tab=` الأولية** (سطران، تحقق من المفاتيح، بلا تغيير سلوك بدونها — نفس نمط FinancePanel القائم) إلى FrontDesk (‏arrivals/departures) وOperations (‏maintenance/housekeeping) وServices (‏orders)؛ تسجيل دفعة → تبويب الدفعات القائم في المالية. **‏Desktop:** سطر مرتب يلتف بأناقة عند الضيق — لا horizontal scroll أبدًا؛ **موبايل ≤640px:** زر «إجراءات سريعة» هادئ يفتح شبكة عمودين — صفر overflow. ترجمات `quickActions` بنصوص المالك الحرفية (14 ×3، تكافؤ **1918**). **صفر Backend** (مؤكد بـ git)، صفر منطق أعمال، لوحة المنصة بلا الشريط (مؤكد بجلستها). لوحة التحكم تبقى للفهم السريع والاختصارات الكبيرة؛ الشريط للعمليات اليومية.
- **لوحة التحكم + نقل الاشتراك إلى التوب بار (بطلب المالك):** (أ) **«لوحة التحكم» أول عنصر** في سايدبار الفندق (يظهر لكل عضو نشط) و**«الاشتراك والباقات» أُزيل من السايدبار** — الصفحة `/hotel/subscription` ومنطق الاشتراك بلا مساس، والوصول إليها صار عبر **badge الاشتراك في التوب بار الذي أصبح قابلًا للنقر** (Link أنيق بإحساس hover — عرض فقط، لا دفع/checkout). (ب) القائمة الرسمية الجديدة (15): لوحة التحكم ثم البنود الـ14 القائمة بترتيبها. أُنشئت وحدة مشتركة `hotelNav.ts` (مصدر واحد للقائمة + فلتر صلاحيات Phase 11 حرفيًا) يستهلكها السايدبار وشبكة الاختصارات معًا. (ج) **صفحة `/hotel` تحولت من redirect للإعدادات إلى لوحة تحكم فعلية**: كروت إحصائيات علوية من **نداء واحد لتقرير Overview القائم (Phase 13) بنطاق اليوم** — إجمالي الغرف/المتاحة/المقيمون الآن/تحتاج تنظيف/تحت الصيانة (لقطات حالية) + حجوزات ودخول ومغادرة ومدفوعات ومصروفات وطلبات خدمة اليوم + مهام التدبير والصيانة المفتوحة (13 كرتًا، StatCard المركزي) — تظهر فقط لحامل `reports.view` (الـ API يفرضها خادميًا أصلًا)؛ وتحتها **شبكة «الوصول السريع»** بكروت اختصار لكل الأقسام الـ14 من نفس القائمة الرسمية وبنفس فلتر الصلاحيات (من لا يملك صلاحية قسم لا يرى كرته) — صفوف أنيقة متجاوبة بلا جداول/BI/منطق مالي جديد. (د) ترجمات: `sidebar.dashboard` + قسم `hotelDashboard` كامل (18 مفتاحًا ×3، تكافؤ **1904**). **صفر Backend** — البيانات كلها من APIs قائمة، والإشعارات بالجرس فقط، والحراس والصلاحيات بلا تغيير. تحقق مصادَق: ‏`/hotel` تعيد 200 بلوحة التحكم (عنوان + اختصارات + هيكل الإحصائيات بالعربي RTL)، ‏overview API بنطاق اليوم يعيد بيانات حقيقية، صفحة الاشتراك 200، ولوحة المنصة سليمة (6 روابط، بلا هوية فندق/badge).
- **سايدبار ثابت بلا تمرير داخلي (بطلب المالك):** السبب الجذري أن `overflow-y: auto` جعل السايدبار **حاوية تمرير داخلية** — حين يفيض المحتوى قليلًا (بعد تكبير الهوية) صارت العجلة فوقه تحرّك القائمة نفسها فبدا «نافذة داخل نافذة». الحل: ‏**`overflow: clip`** على البطاقة (لا scroll ولا scrollbar داخلي ممكنان أصلًا؛ العجلة فوق السايدبار تحرّك الصفحة أي المحتوى فقط، والسايدبار sticky بارتفاع `calc(100dvh − 2×gap)` مسمّر بالكامل — اللوغو والقائمة لا يتحركان أبدًا). ولضمان اتساع الـ15 بندًا بلا تمرير: تكثيف أنيق عام (سطر الرابط 1.3) + **طبقة compact بالارتفاع `@media (max-height: 900px)`** للابتوبات: علامة 2.75rem واسم lg وحواف ومسافات أضيق وفجوة قشرة أصغر — بالحساب المثبت على قيم الـ tokens: العمود الأساسي ~847px (لا يطبق إلا فوق 900px ✓) والـ compact ~639px (يتسع في 1366×768 و1440×900 ✓). **الـ drawer على الموبايل وحده** يحتفظ بتمرير داخلي بشريط مخفي للهواتف القصيرة جدًا. صفر تغيير في القائمة/الترتيب/التسميات/الصلاحيات، والتمرير كله محصور بالمحتوى.
- **تكبير الهوية واستغلال فراغ السايدبار (بطلب المالك):** منطقة Brand حقيقية أعلى السايدبار — علامة المنصة كُبّرت إلى **3.25rem (52px)** بزوايا أكبر وأيقونة أكبر (مقاس `xl` جديد في مكوّن Icon المركزي)، اسم المنصة إلى **xl** فوق سطر النطاق الأوضح (sm)، والمنطقة أعلى بحواف أوسع وفاصل أنظف — والفراغ أسفل السايدبار استُغل **بدون أي عنصر جديد**: حضور أكبر للهوية + مسافات أرحب قليلًا بين روابط القائمة نفسها (صفر تغيير في القائمة/الترتيب/الصلاحيات). في التوب بار: لوغو/مونوغرام الفندق إلى **3rem (48px)** ضمن نطاق المالك المقترح (44–52px) والاسم إلى xl والشريط إلى **5rem**؛ ‏Avatar المستخدم إلى **2.5rem** (مقاس `avatar--lg` جديد؛ بلا إيميل وفتحة الصورة المستقبلية كما هي)؛ زر الخروج صار **بحجم الزر الكامل وبخط عريض** فوق الأحمر الغامق بالنص الأبيض (منطق الخروج بلا مساس). الشخطات ما زالت مخفية عن desktop (إصلاح الخصوصية قائم) وبادج الاشتراك في مكانه. تحقق مصادَق: عربي RTL وإنكليزي LTR على `/hotel/rooms` + لوحة المنصة سليمة (هوية منصة فقط، بلا هوية فندق/بادج) + كل المقاسات الجديدة مؤكدة نصًا في الـ CSS المقدَّم.
- **ترقية التوب بار (بطلب المالك):** ارتفاع أفخم (‏4.75rem مع padding متوازن) · هوية الفندق أكبر وأوضح (لوغو/مونوغرام **2.75rem ≈ 44px** بتدرج العلامة وظلها + الاسم بحجم lg عريض؛ ‏2.25rem على الموبايل والاسم يُخفى ≤420px مع بقاء العلامة) · **أيقونة الثلاث شخطات أزيلت من desktop** — اكتُشف وأُصلح خطأ CSS حقيقي: قاعدة `.icon-btn` اللاحقة كانت تغلب قاعدة الإخفاء فتُظهر الهامبرغر على desktop؛ الآن `.app-topbar .app-sidebar__toggle` تخفيه هناك ويظهر **فقط ≤900px** كزر فتح الـ drawer · ‏Avatar المستخدم كُبّر إلى 2.25rem (فتحة الصورة المستقبلية كما هي، بلا إيميل) · **زر الخروج أحمر غامق بنص وأيقونة بيضاء** (variant ‏danger الصلب — المنطق بلا مساس) · **وbadge اشتراك/باقة جديد** بعد هوية الفندق مباشرة من `profile.subscription_state` القائم (صفر Backend): تجربة → «تجربة مجانية — X يوم» (أزرق) · مدفوع → «اسم الباقة — X يوم» (أخضر؛ بلا نهاية → الاسم فقط) · منتهية (أحمر) · معلّق (أحمر) · بدون اشتراك (رمادي) — الأيام ديناميكية من `days_left` المحسوب خادميًا، ومفاتيح الـ badge الخمسة ×3 لغات (تكافؤ 1886).
...
- الحالة: **مكتملة ✅ ومعتمدة ومدموجة** (جولة إغلاق صغيرة-إلى-متوسطة — ليست مرحلة)
- الدمج: PR #37 · squash · merge commit `4e3e8b4` · اعتماد المالك الصريح («اعتماد تنفيذ جولة إغلاق قسم الإشعارات وبدء بروتوكول الدمج النهائي لـ PR #37 ✅») — الفرع محذوف محليًا وبعيدًا.
