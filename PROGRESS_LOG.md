...
## UI Redesign — «Grand Lobby» (بأمر المالك، عبر إيجنت ui-art-director)
- الحالة: **بانتظار الاعتماد 🔎** (التنفيذ مكتمل — بانتظار المراجعة النهائية من المالك)
- التاريخ: 2026-07-09 (بعد اعتماد Phase 17؛ عمل مُبوّب مستقل — ليس Phase 18)
- الأمر: «استخدم إيجنت ui-art-director لإعادة تصميم واجهة هذا المشروع بالكامل ونفّذ الـ workflow الخاص بك كاملًا» — بعد Readiness Check أكّد خلو الساحة (صفر PRs مفتوحة).

### الاتجاه المُنفّذ
- **«Grand Lobby — الردهة الكبرى»:** كروم زمرّدي داكن (السايدبار، الأبطال، الفوتر، جانب صفحة الدخول) يؤطر أسطح تشغيل عاجية دافئة، بلمسات نحاسية مقتصدة، وعناوين سيريف (Marcellus لاتيني / Cairo ‏700 عربي) فوق Manrope للنصوص — الجداول والنماذج بقيت فاتحة عمدًا (شاشات عمل طوال اليوم).
- **‏Primary الجديد: `#166a53`** (تباين 6.52:1) + نحاسي `#b88a33` (زخرفة) و`#8a6519`/`#d9bd7f` (نصوص AA على فاتح/داكن) + كانفس `#f6f4ee` — كل التباينات مقاسة وموثقة، مع إصلاح fg-subtle القديم الذي كان دون AA.
- **خطوط self-hosted (~94KB):** ‏Manrope متغير + Marcellus + Cairo عربي، بـ unicode-range و`font-display: swap` — و**تصفير letter-spacing تحت `[dir="rtl"]`** على مستوى التوكنات (لا تباعد أحرف للعربية أبدًا).

### النطاق (UI-only مُتحقق منه آليًا)
- **معدّلة:** ‏`tokens.css` (إعادة كتابة كاملة + aliases أصلحت `--color-brand`/`--font-size-md` غير المعرفين) · `globals.css` (سايدبار داكن بشريط نحاسي للنشط، طبقة السيريف، tabular-nums، hover جداول محايد، hero/auth/public، حالة empty نحاسية، إصلاح هيدر الموبايل العام) · `manifest.ts` + `layout.tsx` (‏themeColor) · `offline.html` (ألوان) · `sw.js` (**رفع نسخة الكاش v2 فقط** — سطر واحد، المنطق الأمني كما هو) · **أيقونات PWA الخمس أعيد توليدها** بالهوية الجديدة.
- **جديدة:** ‏`docs/{UI_RESEARCH,UI_BLUEPRINT,DESIGN_SYSTEM}.md` · `fonts.css` + `public/fonts/` (5 ‏woff2).
- **صفر لمس لـ:** ‏backend/ · ‏`lib/api` وعقودها · الجلسات · الحراسات · القواميس (تكافؤ **1856×3 محفوظ** — التصميم CSS بالكامل) · أنماط الطباعة · طبقة استجابة Phase 17.

### الفحوصات (تحقق مستقل من المنسق فوق مراجعة الإيجنت)
| الفحص | النتيجة |
|---|---|
| نطاق الـ diff | ✅ ‏UI-only: ‏backend=0 ملفات، ‏lib=0، قواميس=0 |
| Frontend `lint` / `tsc --noEmit` / `build` | ✅ الكل ناجح (تشغيل مستقل) |
| الخطوط الخمسة + manifest ‏`#166a53` + ‏sw ‏v2 + offline.html | ✅ كلها حية 200 وبالمحتوى الصحيح |
| لقطات بصرية (designqc) | ✅ الرئيسية (ديسكتوب+موبايل)، ‏/hotels، ‏/login — الهوية الجديدة ظاهرة بلا overflow |
| Backend | ✅ غير ملموس إطلاقًا (حالة 681/681 الصباحية قائمة) |

### حدود موثقة
- الفحص البصري للكونسول المصادَق (السايدبار الداكن) ولقطات RTL الفعلية يتطلب جلسة متصفح — التحقق تم كودًا ورياضيًا (تباين) وSSR (`dir="rtl"`)؛ يُنصح بجولة بصرية من المالك في المعاينة قبل القرار.
