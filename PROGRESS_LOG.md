...
## Task — UI Polish & Design System Prototype: جولة الصقل البصري التجريبية
- الحالة: **بانتظار الاعتماد 🔎** (جولة تصميم بصري تجريبية — ليست مرحلة؛ Frontend فقط، صفر backend/API/migrations/صلاحيات)
- **جولة المراجعة 2 (بأمر المالك — المرجع البصري الرسمي):** كروت إحصائية **متدرجة بالكامل** بنص أبيض ودوائر زخرفية وأيقونات دائرية شفافة على ملخصات الغرف والحجوزات وبلاطات الاستقبال (بحلقة تفعيل للفلاتر + aria-pressed)، **صفحة الغرف ضمن النطاق**: رأس بهوية + كروت طوابق بأيقونة دائرية متدرجة وحالة اختيار واضحة + كروت غرف بغسلة لونية وشريط 5px ورقم بارز داخل كتلة ملونة وfooter مفصول، **مودال الفوري/المستقبلي كتلتان متدرجتان كاملتان** (زمردي/إندجو) بنص أبيض، **النزلاء شبكة كروت** بأفاتار متدرج وشريط قمة حسب الحالة (ذهبي VIP/أحمر محظور/زمردي مقيم) وfooter، رقاقات إحصاءات في الملف، وخريطة أزرار العمليات: دخول أخضر/خروج أزرق/تمديد ونقل إندجو/تقصير برتقالي/حذف وردي + متغيرا Button جديدان (info/indigo). لقطات Before/After أُعيد التقاطها كاملة (12×2 بما فيها الغرف وحالتا VIP/محظور الحيتان مؤقتًا ثم أُرجعتا).
- **جولة المراجعة 1 (بأمر المالك — التصميم الأول رُفض لضعف أثره البصري):** تحول مرئي كامل يظهر في اللقطة الثابتة — لوح slate تطفو عليه بطاقات بيضاء بظلال طبقية، رؤوس صفحات بغسلة متدرجة وشريحة أيقونة ملونة وهوية فرعية لكل قسم (حجوزات = إندجو، استقبال = تيل، نزلاء = بنفسجي)، بطاقات KPI بأشرطة قمة ملونة وأيقونات كبيرة، أزرار أطول بألوان الغرض (نجاح متدرج زمردي / تحذير Amber صلب بنص داكن / خطر Rose)، تبويبات حبوب محتواة، رأس جداول مصبوغ + zebra، بطاقة الإقامة برقم غرفة بطل داخل كتلة زمردية، بطاقتا الاختيار بغسلات لونية وأيقونات 3.5rem، نوافذ بنغمات (نجاح/تحذير/خطر) ورأس مميز، أفاتار أحرف للنزلاء، زوايا وظلال أعمق عبر الـ tokens. **لقطات إلزامية نُفذت عبر Playwright + متصفح Edge المحلي بجلسة مصادقة حقيقية** (بلا أي إضافة للمشروع): ‏11 لقطة Before و11 After (فاتح/داكن/موبايل/مودالات) في `.wolf/ui-screens/` — خارج Git.
- التاريخ: 2026-07-11 · الأساس: `origin/main@0c53490` · النطاق: الحجوزات + الدخول والمغادرة + النزلاء (الأساسيات المشتركة تُصقل مركزيًا فتفيد كل الأقسام تلقائيًا)
- **Design Tokens:** البناء على طبقة `tokens.css` القائمة (هوية التيل الفاخرة) بإضافة **لوحة الحالات الفندقية**: محجوز (بنفسجي)، مقيم (زمردي)، مغادر (سلات)، مغادرة اليوم/متسخ (كهرماني)، صيانة (أحمر هادئ)، **VIP (ذهبي رصين)**، محظور (أحمر واضح) — بنسختي soft — + ثوابت التفاعل (`--disabled-opacity`، ‏`--lift-1/-3`، ‏`--shadow-lift`). لا لون مكتوب يدويًا داخل الصفحات.
- **الوضع الداكن (نموذج تجريبي):** المشروع كان فاتحًا فقط — أُضيفت طبقة `[data-theme="dark"]` كاملة لكل الـ tokens (أخضر داكن هادئ، لا أسود صريحًا) **بلا أي تفعيل تلقائي ولا مفتاح** — تُراجَع بوضع السمة يدويًا على `<html>`؛ قرار المفتاح وتعميمه للمالك.
- **نظام الأزرار:** نوعان جديدان — **Success** (دخول/خروج/إتمام: أخضر هادئ) و**Warning** (تمديد/تقصير/نقل: غسلة كهرمانية soft) فوق القائم (primary/secondary/danger/dangerSoft/ghost)؛ رفع hover ‏`-1px` مع ظل ناعم، ضغطة Active حقيقية، ‏focus-visible موحد، ‏disabled بـ token، ‏Loading كما هو.
- **الحركات الدقيقة:** خاصية `anim` جديدة على Button (‏13 نية: add/open/back/edit/save/checkin/checkout/move/extend/shorten/delete/block/vip) — دوران 10° للإضافة، انزياح اتجاهي للدخول/الخروج/النقل (منعكس تلقائيًا في RTL)، تمدد/انكماش للتمديد/التقصير، ‏shake واحد قصير للحذف عند الـ hover فقط، ‏pop واحد لـ VIP — كلها transform/opacity، بلا حركة مستمرة، وبلا 360°/bounce/glow.
- **الكروت:** رفع hover هادئ (−1 إلى −3px + ‏`--shadow-lift`) لبطاقات الإقامة والـ workflow والـ action cards مع إبراز أيقونة الكرت؛ بطاقة الإقامة صارت بشريط زمردي (مقيم) وترقيم tabular؛ **بطاقتا اختيار نوع الحجز** صارتا بطليتين: أيقونة أكبر بحاويات ملونة (فوري = زمردي / مستقبلي = بنفسجي)، شريط جانبي مميز، سهم يظهر عند الـ hover/التركيز (منعكس RTL)، ودعم كيبورد كامل.
- **Badges:** نغمات جديدة `vip` (ذهبي) و`blocked` و`inhouse` و`reserved` + دخول fade+scale لمرة واحدة؛ شارات VIP/محظور/مقيم في النزلاء والاستقبال انتقلت للنغمات الدلالية — الشارة تظهر **فقط** عند الحالة الفعلية.
- **الجداول:** ‏hover أنعم (غسلة أخف)، أزرار الصفوف تتقدم تدريجيًا عند hover الصف، أرقام tabular-nums.
- **‏Dialogs/Toasts/Empty:** دخول المودال الموحد fade+scale ‏0.98→1 مع انزياح 6px بمدة 200ms (الإغلاق فوري)، ‏Toast ينزلق من طرف الشاشة (اتجاهي RTL/LTR)، أيقونة الحالة الفارغة تنبثق مرة واحدة. ‏Skeleton بالـ shimmer القائم دون توسع.
- **إتاحة وأداء:** ‏`prefers-reduced-motion` يلغي كل transform/animation الجديدة ويبقي تغير اللون؛ ‏focus-visible على الأزرار وبطاقات الاختيار؛ ‏transform/opacity حصرًا؛ **صفر مكتبات جديدة** (لا Framer Motion — ‏CSS كافٍ وموثق).
- الربط: ‏13 موضع زر في الأقسام الثلاثة (دخول success+checkin، خروج success+checkout، تمديد/تقصير/نقل warning بأيقوناتها، حظر dangerSoft/danger+block، حذف danger+shake، ‏VIP ذهبي+pop، فتح الملف/التفاصيل open، الحفظ save، الإلغاء delete، الرجوع back، زر «حجز فوري» النهائي في المعالج success+checkin).
- **صفر تغيير منطقي**: لا API، لا backend، لا migrations (مثبت بـ--check)، لا صلاحيات، لا نصوص (تكافؤ الترجمات 2190×3 كما هو). لقطات الشاشة الموثقة تعذرت تقنيًا (صفحات مصادَقة — قيد designqc المعروف)؛ المراجعة الحية على خادم التطوير + وصفة معاينة الداكن في التقرير.
