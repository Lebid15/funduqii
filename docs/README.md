...
- [MOBILE_PWA_PERFORMANCE_STRATEGY.md](MOBILE_PWA_PERFORMANCE_STRATEGY.md) — the central mobile/tablet polish layer (what already existed vs what Phase 17 added; token-only, RTL/LTR-safe), the PWA foundation (manifest/icons/metadata, installability), the safe-offline line (static trilingual fallback only; why API/page caching, offline writes and cached tokens are refused), the performance fixes (public-list subscription batch with parity test, media prefetch, platform-list select_related, the enforcement index), what was checked, and what is deferred (full offline, push, virtualized tables, image pipeline).

## Phase 16 — Platform Owner Panel Completion

- [PLATFORM_OWNER_PANEL_STRATEGY.md](PLATFORM_OWNER_PANEL_STRATEGY.md) — platform owner vs hotel user separation, the audited hotel lifecycle (suspend requires a reason; no hard delete), completed plans (reuse-first; deactivate-not-delete), the subscription lifecycle (trial once-only as the FIRST subscription, manual paid activation without any gateway, renew/cancel/expire with preserved history), manual platform payments (void-not-delete, separate from hotel finance), the ONE central subscription-enforcement chokepoint (what is blocked vs what always stays readable; time-aware; suspension wins), the hotel-console banners, public-site admin settings (not a CMS; per-locale overrides with dictionary fallbacks; safe URLs), the dashboard's estimated-recurring-revenue rules (never "profit"), and everything deferred (gateway, accounting, CRM, OTA, Phase 17).
